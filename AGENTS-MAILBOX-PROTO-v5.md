...
Protocol version: **5**
Status: **candidate; activates only after the protocol-v4 mailbox is drained**
...
- `send TO_ROLE DESTINATION [BODY_FILE|-]`: publish a durable directed handoff by default; `TO_ROLE=all` publishes a durable broadcast notice;
...
Protocol-v3 `REVIEW-PENDING`, `IMPL-PENDING`, `APPROVAL-PENDING`, and doubled-separator `CLAIMED--*` files are not valid v5 work and cannot be consumed by Baton. Protocol-v4 envelopes are likewise not valid v5 messages. `doctor` reports recognized incompatible state as requiring explicit human recovery. V5 provides no legacy publication mode, compatibility alias, or dual reader.
...
Version 5 adds explicit durable/transient retention and formal message-stream semantics to the configured-role, full-`FROM/TO` protocol. Its local trial in `drift-lang` begins only after the v4 mailbox drains and the v5 cutover completes. Peer repositories remain on their presently distributed protocol until Slawomir explicitly approves v5 distribution.
