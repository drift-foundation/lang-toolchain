...
- Program entry (v1): exactly one `main` function, returning `Int`, **declared `pub` and `nothrow`**, with one of two signatures:
  - `pub fn main() nothrow -> Int`
  - `pub fn main(argv: Array<String>) nothrow -> Int` (argv includes the program name at index 0). The runtime builds `argv` and calls this `main`; no drift_main indirection in user code.
  ...
  - A private (non-`pub`) `main` is rejected at typecheck for any real app build (`entrypoint main must be declared pub`).
...
The package trust model is defined in
[`doc/design/trust-v1.md`](trust-v1.md), which is the authoritative
contract; this section gives the spec-level summary.

Signatures are generated by tooling, not by `driftc`.  Two distinct
tools sign two distinct claims, on two distinct keys (the *role
split*):
...
- **Cert claim** (`<pkg>.cert-claim.<kid>.json`) — emitted by
  ...
  dep_graph, cert_suite, run_id, run_started_utc,
  evidence_sha256)`.  One sidecar file per independent certifier
  (filename embeds the kid).

Both claims cover canonical JSON bodies (not the `.dmp` bytes
directly).  The cert claim's `artifact_sha256` field is the
binding to the actual `.dmp` bytes, which are the canonical
uncompressed DMIR-PKG container bytes (transport compression like
`pkg.dmp.zst` is outside the signed payload).

**Verification point.** Both claims are verified at module
import / compilation time by the Drift toolchain
(`provider_v1.load_package_v1_with_policy` → `verify_v1.
compose_verify`).  No runtime signature verification is performed
by the generated program.

**Policy rule.** Both claims are required for every consumed
package (the dev-time `allow_unverified_roots` opt-in is gated
against the reserved namespaces `std.*`, `lang.*`, `drift.*` and
is intended for local-only development).  There is no pre-v1
`.sig` envelope acceptance path.
...
This specification focuses on semantics: ownership, types, errors, concurrency, and runtime behavior. The complete formal grammar (tokens, precedence, productions) lives in `doc/design/drift-lang-grammar.md` and is authoritative for syntax. In case of conflict: semantics in this spec win for meaning; syntax in the grammar file wins for how code is parsed.
