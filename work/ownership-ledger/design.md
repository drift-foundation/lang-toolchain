...
4. **Driver-side `needs_drop` approximation (4b residual).** The driver's reporter callable in `driftc.py` uses raw `TypeTable.has_drop` rather than `DropPolicy.needs_drop` because the driver does not hold a HIRToMIR handle at the ledger-build point. Diverges on `copy_status=True` and DV-bearing types — both surface in 3A telemetry under the dedicated triage bucket (see `4b-smoke-observations.md`). The three-quadrant pin uses the real `DropPolicy.needs_drop` so the gate is not vulnerable to this approximation, but **3B MUST replace this with a per-function DropPolicy accessor before any consumer is swapped onto the ledger** — reusing `has_drop` in a non-observational pass would re-open the Phase 2a UAF surface. Quarantined comment lives at the call site in `driftc.py`.
