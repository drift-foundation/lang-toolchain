...
- Codegen: `lang2/codegen/llvm/` now exposes an SSA-first textual LLVM emitter seeded with the v1 ABI types (`%DriftError`, `%FnResult_Int_Error`). The primary API is `lower_ssa_func_to_llvm(func, ssa, fn_info, fn_infos)`; MIR-based lowering was removed. SSA now carries `block_order` (deterministic RPO) and `cfg_kind` to make CFG support explicit. Control flow is still limited to straight-line/diamonds (loops rejected). Smoke tests live in `lang2/codegen/tests/test_llvm_codegen_basic.py` and are wired into `just lang2-codegen-test` (included in `lang2-test`). End-to-end clang/lli runs remain TODO.
