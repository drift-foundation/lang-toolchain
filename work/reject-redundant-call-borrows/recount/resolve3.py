...
class U: pass
units=[]
for uid,rec in enumerate(raw):
    path,area,kind,off,err,mode,prog,src = rec
    u=U(); u.uid=uid; u.path=path; u.area=area; u.kind=kind; u.off=off; u.err=err; u.prog=prog; u.mode=mode; u.src=src
...
def scan(u,fname,params,block,tparams,selft):
    ...
    for n in nodes:
        ...
        if pending:
            ...
            elif isinstance(recv,A.Name) and FREE.get(cname):
                # fragment without visible import: `alias.fn(...)` resolves to a free fn by name
                fc=FREE[cname]
                ismem=any(d.unit.path.endswith('std/mem/mem.drift') for d in fc) and cname in MEM_INTRINSICS
                fc2,conf=narrow(fc,u,nargs,False)
                for i,k,a in pending:
                    if classify(ctx,a,fc2,i,k,False,('mem' if ismem else 'free'),cname,conf+'/modguess')=='arity':
                        unresolved['arity-modguess:'+str(cname)]+=1; unresolved_sites.append((u.path,line,'arity-modguess',str(cname)))
            else:
...
json.dump(results,open(S+'/results3.json','w'))
json.dump({'unresolved':dict(unresolved),'excluded':dict(excluded),'total_borrow_args':total_borrow_args,
 'macro_borrows':macro_borrows,'ambiguous_sites':ambiguous_sites,'unresolved_sites':unresolved_sites},open(S+'/diag3.json','w'))
