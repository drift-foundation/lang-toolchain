...
- **Defect**: `drift prepare` does not recognize co-artifacts in the same manifest.
  - When a manifest declares multiple artifacts and one depends on another (e.g. `web-rest` depends on `web-jwt`, both in `drift-manifest.json`), `drift prepare` searches `--package-root` for a published `.zdmp` and fails if the dep hasn't been deployed yet.
  - Expected: `drift prepare` should recognize that a `package_dep` is satisfied by a co-artifact in the same manifest and either skip external resolution or synthesize a lock entry from the manifest version.
  - Workaround: deploy the dependency artifact first (`drift deploy --artifact web-jwt`), then run `drift prepare` for the full project.
  - Affects any multi-package project with intra-project dependencies.
  - Filed: 2026-03-19.
