# lang directory layout

- `core/`: shared core shims (diagnostics, TypeTable/TypeEnv protocol, TypeEnv test impls). Tests in `core/tests/`.
- `parser/`: self-contained copy of the Drift parser + grammar (no runtime deps on `lang/`), with adapter to `lang.driftc.stage0` AST. Tests in `parser/tests/`.
- `stage0/`: lang AST definitions (surface syntax mirror for the refactor).
...
- `lang-test`: runs all stage/unit/integration/core/driver/codegen + parser tests.
- `lang-parser-test`, `lang-core-test`, `lang-driver-test`, `lang-codegen-test`: individual suites.

Artifacts:
- Test artifacts (codegen) go under `build/tests/lang/...` and are cleaned by `lang-codegen-test`.
