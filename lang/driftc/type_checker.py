...
class TypedFn:
	...
	# coercion at a specific call site, NOT a global Ptr/Ref type equivalence.
	# Lowering needs no transform (both are `ptr` values).
	ptr_to_ref_coercions: Dict[int, TypeId] = field(default_factory=dict)
...
class TypeChecker:
	...
	a small set of builtin constructs (f-strings, exceptions).
	"""
	...
	def __init__(self, type_table: Optional[TypeTable] = None, *, allow_unsafe: bool = False, allow_unsafe_without_block: bool = False, unsafe_trusted_modules: set[str] | None = None, pkg_unsafe_modules: set[str] | None = None, semantic_world: object | None = None, source_modules: set[str] | None = None):
		...
		# Slice 7c-3 (ABI 14, 2026-05-06): `_dv` cache + DV TypeId gone
		# along with `TypeKind.DIAGNOSTICVALUE` and the public DV
		# substrate.  Former call_ctx `dv_ty` field also retired.
	...
	def find_thunk_spec_by_id(self, thunk_fn_id: FunctionId) -> ThunkSpec | None:
		"""Reverse lookup for a previously-registered OK-wrap /
		boundary thunk.  Used by the `callback{N}` resolver to
		recover the original `target_fn_id` + declared signature
		when an `HFnPtrConst` arg carries a thunk-kind `fn_ref`.
		"""
		for spec in self._thunk_specs.values():
			if spec.thunk_fn_id == thunk_fn_id:
				return spec
		return None
	...
	def validate_no_recursive_value_types(self, *, diagnostics: list[Diagnostic]) -> None:
		...
		def _suggest_indirection(field_decl_type_expr: object | None, self_type_name: str) -> str:
			...
			The primary suggestion is `Box<Self>` — the unique-ownership,
			value-semantic heap indirection (`core.Box`), which is the natural
			fix for breaking a value-type layout cycle.  (`Arc<Self>` is the
			shared-ownership alternative when aliasing is wanted.)

			- If the field type is `Optional<...Self...>`, suggest
			  `Optional<Box<Self>>` (preserving the user's wrapper).
			- Otherwise the primary suggestion is `Box<Self>`.
			"""
			if field_decl_type_expr is not None:
				expr_name = getattr(field_decl_type_expr, "name", None)
				if expr_name == "Optional":
					return f"Optional<Box<{self_type_name}>>"
			return f"Box<{self_type_name}>"
		...
		for cycle in cycles:
			...
			if len(cycle) > 1:
				message = (
					...
					f"indirection (Box, Arc, &, Array, RawPtr)"
					f"{suggestion_phrase}"
				)
			else:
				message = (
	...
	def validate_trait_impls(
		...
		*,
		...
		# Slice 7c-3 (ABI 14): `TypeKind.DIAGNOSTICVALUE` deleted —
		# `String` is the only remaining compiler-known Copy scalar.
		_COMPILER_KNOWN_COPY_KINDS: set[TypeKind] = set()
		...
		def _is_structurally_copy(tid: TypeId, *, seen: set[TypeId], covered_tparams: frozenset[TypeParamId] = frozenset()) -> bool:
			...
			if td.kind in (TypeKind.ARRAY, TypeKind.FNRESULT, TypeKind.ERROR, TypeKind.UNKNOWN, TypeKind.FORWARD_NOMINAL):
				return False
	...
	def check_function(
		...
		# Typed-catch binders are FUNCTION-scoped: binding ids are reset per
		# function (see `_next_binding_id` reset below), so an accumulating
		# dict would let one function's catch binder id collide with an
		# unrelated binding in the next, falsely tagging it as catch-provenance
		# (which previously cascaded false constructor mismatches into stdlib).
		# Reset at entry so `self._typed_catch_binders` only ever holds the
		# current function's binders.
		self._typed_catch_binders = {}
			...
			if hasattr(H, "HPlaceExpr") and isinstance(expr, getattr(H, "HPlaceExpr")):
				...
				for idx, proj in enumerate(expr.projections):
					...
					if sub_def.kind is TypeKind.ERROR:
						...
						# Slice 7c-3 (ABI 14, 2026-05-06): with the DV
						# substrate gone, this path only fires after the
						# upstream `E_EXC_ATTRS_REMOVED` rejection has
						# been emitted in HField/HIndex.  Return type is
						# `Unknown` — compilation has already failed and
						# no later check consumes the result.
		...
		# Symmetric `&T → T` / `&mut T → T` coercion: a separate
		# (secondary) check the free-function / method resolver runs
		# ONLY when strict `_args_match_params` finds no candidates.
		# Mirrors the borrowed field-projection auto-dup at
		# type_checker.py:8645-8646, but kept out of the strict path
		# so existing overload disambiguation (`pick(&s)` choosing
		# `pick(s: &String)` over `pick(s: String)`) keeps working —
		# the exact `&T → &T` match must win.
		def _can_ref_to_value_coerce(params: list[TypeId], args: list[TypeId]) -> bool:
			if len(params) != len(args):
				return False
			saw_ref_to_value = False
			for param_ty, arg_ty in zip(params, args):
				if arg_ty is None:
					return False
				param_cmp = _dealias_zero_param_type(param_ty)
				...
				if (
					...
					continue
				if (
					arg_def.kind is TypeKind.REF and param_def.kind is not TypeKind.REF
					and arg_def.param_types and arg_def.param_types[0] == param_cmp
				):
					inner = param_cmp
					if self.type_table.copy_status(inner) is True or _proves_const_share(inner):
						saw_ref_to_value = True
						continue
				return False
			return saw_ref_to_value
		...
		def _apply_autoborrow_args(
			...
			*,
			...
			for idx, (param_ty, arg_ty, arg_expr) in enumerate(zip(param_types, arg_types, args)):
				...
				ref_info = _ref_param_info(param_ty)
				if ref_info is None:
					# Symmetric arg coercion: &T → T (and &mut T → T)
					# when T is Copy or proves ConstShare.  Mirrors
					# borrowed field-projection auto-dup at
					# type_checker.py:8645-8646.  Inserts an explicit
					# deref HIR node; for non-Copy ConstShare T, also
					# wraps in `.const_share()` so HIR→MIR sees the
					# fully-resolved owned-value shape.
					arg_def = self.type_table.get(arg_ty)
					if (
						...
						if self.type_table.copy_status(inner) is True or _proves_const_share(inner):
							...
							coerced_ty = type_expr(deref_expr, expected_type=inner)
							if coerced_ty == inner:
								args[idx] = deref_expr
								updated_types[idx] = coerced_ty
								# For non-Copy ConstShare T, wrap the
								# deref node in `.const_share()` so
								# HIR→MIR sees the fully-resolved owned
								...
								# `_is_suppression_marked` helper
								# (peer-closure name resolution).
								if (
									self.type_table.copy_status(inner) is not True
									and _proves_const_share(inner)
								):
									wrap = H.HMethodCall(
										receiver=deref_expr,
		...
		def _call_sig_for_fn_ref(sig: FnSignature) -> tuple[list[TypeId], TypeId, bool] | None:
			...
			if getattr(sig, "is_exported_entrypoint", False) or getattr(sig, "is_extern", False):
				# Exported / extern fn-refs flow through the OK-wrap or
				# boundary thunk at the construction site (lines
				# 3395-3400 below), which canonicalizes the cross-
				# package ABI to `FnResult<T, Error>`.  The TypeId
				# must report `can_throw=True` so direct call sites
				# like `val fp = a.id; try fp(1) catch {...}` keep
				# their FnResult unwrap semantics intact.
				#
				# The callback-wrap path (`core.callback{N}(pkg.fn)`)
				# requires the user-declared `nothrow` bit instead —
				# `Fn{N}.call` is declared nothrow, and the wrap
				# embeds the original function (not the OK-wrap thunk)
				# in the Fn{N} interface impl, so FnResult adaptation
				# is irrelevant.  That site is handled separately at
				# `call_resolver.py` in the `callback{N}` intrinsic
				# resolver: when the side-table entry in
				# `fnptr_consts_by_node_id` for the arg has
				# `kind=THUNK_OK_WRAP`, the resolver rewrites the
				# entry in place to point at the underlying impl
				# (recovered via `find_thunk_spec_by_id`) before
				# the `fn_throws` check.  `_apply_fnptr_consts`
				# later materializes the rewritten entry, so the
				# `HFnPtrConst` that reaches HIR→MIR carries the
				# original-function `FunctionRefId` (IMPL kind),
				# not the thunk.
				can_throw = True
		...
		def type_expr(
			...
			*,
			...
			if hasattr(H, "HQualifiedMember") and isinstance(expr, getattr(H, "HQualifiedMember")):
				...
				def _lower_generic_expr(expr: GenericTypeExpr) -> TypeId:
					...
					if alias_def is None and not _name_qualified and origin_mod is not None and not (
						self.type_table.get_struct_base(module_id=origin_mod, name=str(name))
						or self.type_table.get_variant_base(module_id=origin_mod, name=str(name))
						or self.type_table.get_interface_base(module_id=origin_mod, name=str(name))
					):
						# Bare-name precedence: a module-local nominal outranks a
						# UNIQUE cross-module re-export alias (a user `struct Box`
						# must not be hijacked by `std.core`'s re-exported `core.Box`).
						# Consult the unique-alias fallback ONLY when the name was
						# unqualified AND origin_mod declares neither an exact alias
						# nor an exact nominal of this name.
			...
			if isinstance(expr, H.HIndex):
				# Slice 7c-3 (ABI 14, 2026-05-06): the legacy
				# `Error.captures["frame"]["key"]` user-source path is
				# rejected upstream with `E_EXC_CAPTURES_REMOVED` and the
				# match here exists only to attach diagnostics (key-type
				# checks, "captures only on Error" guard).  The fall-through
				# return type is `Unknown` because compilation has already
				# failed; users now use `e.context.encode_compact()` for
				...
				if (
					...
					return record_expr(expr, self._unknown)
				# Slice 7c-3 (ABI 14, 2026-05-06): the legacy
				# `Error.attrs["key"]` user-source path is rejected
				# upstream with `E_EXC_ATTRS_REMOVED`; the match here only
				# attaches diagnostics (key-type check, "attrs only on
				# Error" guard) and returns `Unknown` after compilation
				# has already failed.  Users now use
				# `e.params.get(k).as_*()` to project params.
			...
			# Slice 7c-3 (ABI 14, 2026-05-06): the user-source
			# `e.attrs(...)` call form is rejected here; the legacy
			# attrs surface is fully retired (see
			# `E_EXC_ATTRS_REMOVED` for the indexed `e.attrs[...]`
			# form).  Throw-side params are now owned by
			# `core.Diagnostic.to_json_text(&E)` + `ExcSetParamsJson`.
			if isinstance(expr, H.HCall) and isinstance(expr.fn, H.HField) and expr.fn.name == "attrs":
				diagnostics.append(
					_tc_diag(
						message=(
							"`Error.attrs(...)` is removed in 0.31.62; throw-side "
							"params projection is owned by `core.Diagnostic for E` — "
							"declare `pub error E { ... }` (auto-synthesized) or "
							"`implement core.Diagnostic for E { fn to_json_text(self) -> String { ... } }`"
						),
						severity="error",
						span=getattr(expr, "loc", Span()),
						code="E_EXC_ATTRS_REMOVED",
			...
			if isinstance(stmt, H.HLet):
				...
				if bool(getattr(stmt, "capture", False)):
					...
					if cap_ty is not None and cap_ty != self._unknown:
						allowed = {self._int, self._uint, self._bool, self._string, self._float}
						if cap_ty not in allowed:
							diagnostics.append(
								_tc_diag(
									message="captured locals currently support Int/Uint/Bool/Float/String only",
									severity="error",
									span=getattr(stmt, "loc", Span()),
								)
							)
				...
				if cap_bid is not None and cap_name is not None:
					...
					if cap_kind == "ref":
						...
						return
				_assign_value_ty = type_expr(stmt.value)
				_assign_target_ty = type_expr(stmt.target, used_as_value=False)
				# Symmetric `&T → T` coercion at the assignment slot
				# (mirrors the 0.31.75 let-init / return / binop paths).
				# Pre-fix, `obj.field = ref_string` (target field
				# `String`, RHS `&String`) slipped past the checker
				# and surfaced in HIR→MIR / LLVM lowering as
				# `StoreRef value type mismatch (have ptr, expected
				# %DriftString)`.  Rewrite the RHS to insert a
				# `HUnary(DEREF)` (+ const_share wrap for non-Copy
				# ConstShare T) so the lowering sees a matched-type
				# store.
				if (
					_assign_value_ty is not None
					and _assign_target_ty is not None
					and _assign_value_ty != _assign_target_ty
					and _ref_to_value_coerce_applies(_assign_value_ty, _assign_target_ty)
				):
					_rewritten_rhs = _rewrite_ref_to_value(stmt.value, _assign_target_ty)
					if _rewritten_rhs is not None:
						stmt.value = _rewritten_rhs
						_assign_value_ty = _assign_target_ty
				# Reject any remaining ref-to-value mismatch with a
				# friendly diagnostic before the type / shape mismatch
				# surfaces in lowering (the prior failure mode was an
				# internal codegen ICE — `StoreRef value type
				# mismatch`).
				if (
					_assign_value_ty is not None
					and _assign_target_ty is not None
					...
					_v_def = self.type_table.get(_assign_value_ty)
					_t_def = self.type_table.get(_assign_target_ty)
					if (
						_v_def.kind is TypeKind.REF
						and _v_def.param_types
						and _v_def.param_types[0] == _assign_target_ty
						and _t_def.kind is not TypeKind.REF
					):
						_pretty_t = self._pretty_type_name(_assign_target_ty, current_module=current_module_name)
						diagnostics.append(
							_tc_diag(
								message=(
									f"cannot assign reference value to '{_pretty_t}' field: "
									f"type '{_pretty_t}' is not Copy (use move <expr>)"
								),
								severity="error",
								span=getattr(stmt, "loc", Span()),
							)
