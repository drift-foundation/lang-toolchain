...
def test_method_require_copy_on_generic_param_reports_diag_instead_of_crash(tmp_path: Path, capsys: pytest.CaptureFixture[str]) -> None:
...
implement R {
	fn keep<T>(self: &R, value: T) nothrow -> Int require T is core.Copy {
		val _ = value;
		return 7;
	}
}
