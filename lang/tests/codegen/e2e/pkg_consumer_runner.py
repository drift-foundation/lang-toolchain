...
def _run_case(
	...
	# the in-process and PEX runners' contract — only the debug-style lane
	...
	if required_lane == "debug-style" and not (os.environ.get("DRIFT_DEBUG") in ("1", "true", "True")):
		return case_name, True, "", "skipped (requires debug-style lane)"
