"""
LLVM IR runner for lang codegen IR cases (clang-only).

Each test case lives under `lang/codegen/ir_cases/<case>/` and should contain:
...
BUILD_ROOT = ROOT / "build" / "tests" / "lang"
...
Run a single codegen IR case:
	...
	Note: until the lang parser/CLI is wired, tests are expected to supply
...
def main(argv: Iterable[str] | None = None) -> int:
	ap = argparse.ArgumentParser(description="Run lang codegen IR cases (clang-based)")
	ap.add_argument(
		"cases",
		nargs="*",
		help="Specific test case names to run (directories under lang/codegen/ir_cases)",
	)
	args = ap.parse_args(argv)

	case_dirs = sorted((ROOT / "lang" / "codegen" / "ir_cases").iterdir())
	if args.cases:
