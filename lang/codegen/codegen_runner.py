...
"""
lang-only e2e runner.

Default layout: each case lives under `tests/lang-e2e/<case>/` with:
  - main.drift
  - expected.json

This runner drives `lang.driftc` for both compile-error (JSON diagnostics)
...
BUILD_BASE = REPO / "build" / "tests" / "lang" / "e2e"
...
def main(argv: Iterable[str] | None = None) -> int:
	ap = argparse.ArgumentParser(description="Run lang e2e tests via lang.driftc")
	ap.add_argument("cases", nargs="*", help="Specific test case names (subdirs under --root)")
	ap.add_argument(
		"--root",
		default="tests/lang-e2e",
		help="Root directory containing per-case subdirs (default: tests/lang-e2e)",
	)
