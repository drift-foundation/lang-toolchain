...
- Finding handoffs must follow `AGENTS-MAILBOX-PROTO.md`. Read that versioned protocol in full before publishing or consuming a handoff; it supersedes any older token convention in repository notes or finding artifacts. Use `tools/baton/baton` for mailbox discovery, waiting, claims, publication, responses, and pops; do not manually scan `work/mailbox/*` or recreate polling/claim loops. If Baton refuses or appears defective, preserve the exact filesystem state and report the defect rather than manually repairing, editing, removing, or requeueing a token.
