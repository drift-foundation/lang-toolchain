...
- Finding handoffs must use Baton protocol 6. Before publishing or consuming a handoff, locate the locally deployed Baton distribution and read its `AGENTS-MAILBOX-PROTO.md` in full; it supersedes older conventions in repository notes or finding artifacts. The deployment supplies the executable and an explicit absolute config path—never infer either from repository layout or hard-code a host-specific path. The SQLite instance is the only authority: never claim through another domain's participant, touch the database with raw SQL, or attempt manual state repair. If Baton refuses or appears defective, preserve the exact instance state and report the defect through a finding.
- This project's Baton identities are `lang.reviewer` for the reviewer and `lang.implementer` for the implementer. Resolve role-only instructions to those identities; do not substitute a participant from another domain.
