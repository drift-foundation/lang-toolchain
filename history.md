...
- **Phase 0 — v2 vocabulary/layout break (source-identity change → pool re-cert):**
  - Canonical artifact kind flips `library → package` (these are importable Drift packages that may carry SQL/schema/doc ASSETS, not C-style libraries). `manifest.py` reverses its normalization (`library` is now the deprecated parser alias → `package`); `compute_artifact_sci` passes `art.kind` (so **apps carry SCI too**). `kind` is hashed into `source_content_id`, so every importable artifact's SCI changes — a deliberate pool-wide re-cert. The low-level `compute_source_content_id` now **rejects** any non-canonical kind (`library` never reaches the signed identity; manifest parsing is the sole alias boundary).
  - Deploy category `lib/ → pkg/` (+ `app/` already existed); `--dest`/`--app-dest` are caller-supplied, so this is convention/docs/ORCH coordination, not publish-code surgery. `discover_package_files` globs given roots (layout-agnostic) → consumers just point `--package-root`/`$DRIFT_PACKAGE_ROOT` at `pkg/`. `DRIFT_PACKAGE_ROOT` stays canonical (the stray `DRIFT_PKG_ROOT`/`lib` doc refs were fixed). Toolchain `bin/lib` install path and `/var/lib`/`stdlib` are intentionally untouched. **No `doc` artifact kind** — docs/SQL/templates are declared assets on a package or app.
  - All `art.kind == "library"` checks swept to `"package"` across deploy/build/prepare/author/trust + stdlib deploy SCI; help text/messages/comments and design docs (`trust-v1.md`, `toolchain-build-workflow.md`, `provenance-bundle.md`) canonicalized.
- **Phase 1 — certified app artifacts:**
  - **Signed-schema bump (clean break, no legacy reader):** author-claim + cert-claim BODY `schema_version 1 → 2` with a REQUIRED `artifact_kind` (`"package"|"app"`); the cert body adds a signed `artifact_path` (the deploy-relative locator — `<pkg>.zdmp` for a package, the binary for an app — held to canonical spelling so one artifact has exactly one signed locator). Provenance `schema_version 3 → 4` with a REQUIRED `source_content_id` (the third SCI leg). All three carry the same canonical-kind + `validate_sci` checks at the producer. Emitters build bodies via new `make_author_claim_body`/`make_cert_claim_body` factories that stamp the schema version internally (kills the stale-local-`_BODY_SCHEMA_VERSION` drift class — a static regression pins it).
  - **Deploy emission** moved fully to v2/v4 across all producers (stdlib, `drift author` publish + publish-raw, `cert` publish CLI, `drift deploy` cert emit); SCI now fails hard before any provenance is built for both package and app (no warn/skip).
  - **Consumer verify cross-checks (`verify_deployed_v1`):** package path enforces author == cert artifact_kind == `"package"`, cert `artifact_path` == deployed `.zdmp` filename, cert/provenance `artifact_sha256` == deployed, author == cert == provenance `source_content_id` (**no two-way fallback** — missing/malformed provenance SCI is a hard fail), provenance `artifact_kind`/`artifact_name`/`artifact_version`/`schema_version==4` all match. New `verify_deployed_app` adds the analogous app path: reads the author claim for the app id/namespace, derives a single synthetic trust subject from the declared namespace prefix, locates the binary by the signed `artifact_path` (rejecting symlinks), runs `compose_verify` with the subject, and enforces the same three-leg `"app"` agreement.
  - **`drift verify-app <app-dir>`** — read-only consumer CLI (same trust flags / JSON / exit codes as `drift verify-package`). **Never executes the binary**: app execution stays the orchestrator/service-manager's job after it independently verifies. There is no `drift run` / verify-then-exec command (explicitly out of scope).
- **Command surface — artifact verification moved to the top level (same slice, no extra bump):** package verification is now **`drift verify-package`** (canonical, sole surface; peer of `drift verify-app`), wrapping the `verify_deployed_v1` facade in `tools/drift_deploy/verify_package_cli.py`. The old **`drift trust verify-package` was removed** (parser + handler) — `drift trust` is for trust management/preflight, not artifact verification. The import-boundary test was tightened (the CLI layer must NOT import the verifier facade directly; it lives in `tools/`). Docs (`trust-v1.md` §7.6/§7.7, workflow, tooling) + helper docstrings canonicalized; the durable contract now documents the app on-disk shape + verify-app flow and the package-only scope of the §3/§4/§7.6 package sections.
- **Deploy-recipe fix (Phase 0 fallout):** the `deploy-prepublish-stdlib-author` justfile recipe hard-coded `kind="library"` into `compute_artifact_source_content_id`, which the new SCI hardening rejects → fixed to `kind="package"` (stdlib is an importable package; SCI now matches the deploy cert step). This was inline Python in the justfile, missed by the docs/tools/test sweep.
- **Test-fixture migration (no contract change):** all v1 `AuthorClaimBody`/`CertClaimBody(schema_version=1,…)` test fixtures across `lang/tests/packages` + `lang/tests/driver` (~22 files) moved to the v2 factories (`make_*_claim_body`), including function-local import blocks; deployed-layout fixtures' signed `artifact_path` aligned to the actually-staged file (`.dmp` for stage-uncompressed consumer-load fixtures, `.zdmp` for the 3 that deploy compressed). Full driver suite + signed-stdlib package-consumer e2e (`pkg_consumer_runner`, `pkgb_runner` incl. the `*_string_heap` ownership cases) green.
- **Version:** `DRIFTC_VERSION` 0.33.56 → **0.33.57**; **`DRIFT_RT_ABI` stays 18** (deploy tooling + claim/provenance sidecar schema + verify + CLI; no runtime boundary, no container `format_version` change). **Pool re-cert required:** the SCI-kind flip changes source identity, so all importable artifacts re-issue v2 author + cert claims (re-deploy through cert); v1 claims and v3 provenance are rejected cleanly. Author-claim re-issue is needed here (unlike asset-only changes) because the SCI itself changed.
- **Regressions:** schema/emit — `test_author_claim_v1` / `test_cert_claim_v1` (v2 bodies + new-field accept/reject), `test_provenance` (v4 + canonical-field rejects), `test_no_v1_claim_constructors` (static: no production v1 ctor / local schema constant), `test_source_content_id` (kind reject). Verify — `test_trust_verify_package_cli` (package cross-checks: kind/path/sha/SCI/name/version/schema-version mismatch + v1/v3 reject + the author-kind dead-check fix), `test_verify_app` (app happy + three-leg disagreement + symlink-binary + trust-before-IO usage-error + CLI). Existing `verify_v1`/`v1_adversarial`/`unpack` suites stay green. Plan/progress: `work/certified-app-artifacts/`.

## 2026-06-24 - 0.33.56: cryptographically-verified package assets — `drift deploy` packs declared assets into the `.zdmp`; new `drift unpack` (FEATURE)
### FEATURE: declared package assets travel INSIDE the verified container + a verify-gated `drift unpack`
- **Motivation:** drift-workflows (Singular/Microflows) ship DB schema (SQL applied by Mariachi) as part of a certified library's runtime contract, but would not publish loose, unverified asset files. A cert-only consumer (bookkeeper) had no sanctioned, reproducible way to obtain that schema from the certified path. Generic consuming tools never run package verification — so the only safe design is: assets live in the cryptographically-validated `.zdmp`, materialized through a verify-gated unpack.
- **Key pre-existing substrate (not re-invented):** asset `(path, sha256)` were ALREADY folded into `source_content_id` (`source_content_id.py::compute_artifact_source_content_id`), and SCI is bound three-ways across the `.dmp` manifest + author-claim + cert-claim. So asset *content* was already committed under both signatures; what was missing was (a) the bytes traveling inside the cert-claim's `artifact_sha256` container, and (b) a sanctioned verify-then-materialize command. Assets were previously staged as a loose `assets/` folder beside the `.zdmp` (no integrity of its own).
- **D1 — pack assets into the container** (`lang/driftc/driftc.py`, `tools/drift_deploy/build_cmd.py`, `lang/driftc/packages/dmir_pkg_v0.py`): `driftc --emit-package` gains a repeatable `--asset RELPATH FILE`; each asset file's bytes become a content-addressed blob (new `BLOB_TYPE_ASSET = 3`) in the SAME `manifest.blobs`/TOC bookkeeping as module/interface blobs, referenced by a new `manifest.assets: [{path, blob, len}]` section. Logical paths are normalized with the SAME validator the SCI uses (`_normalize_canonical_path`: no absolute, no `..`), so the in-container path matches the signed source identity and can never escape the consumer's `--dest`. `build_package_cmd` passes one `--asset` per declared `artifacts[].assets[]` entry (covers both `drift build` and `drift deploy`). Assets are now covered by BOTH `artifact_sha256` (cert claim, over the whole `.dmp`) and the SCI commitment — a tampered asset breaks the per-blob sha at load AND the signed artifact hash at verify.
- **Loose folder dropped** (`tools/drift_deploy/drift_deploy.py`): a LIBRARY package no longer publishes a loose `assets/` folder (the ambiguous trust path is removed deliberately). Apps — which have no signed container to carry assets — keep the loose staging (app asset trust is out of scope for the in-package design).
- **D2 — `drift unpack <pkg-dir> --dest <dir>`** (new `tools/drift_deploy/drift_unpack.py` + `lang/drift/cli.py` dispatch): verify FIRST via `verify_deployed_package` (author+cert signatures, SCI three-way, artifact hash, provenance) — write NOTHING on failure. Trust source MUST be supplied (mirrors `drift trust verify-package` flags: `--trust-store` / `--author-profile` / `--author-pubkey-b64` / `--allow-bundled-pubkey`); there is no silent self-trust fallback. On full pass: decompress, extract each asset to a sibling TEMP dir, re-check its sha256 and re-validate its path stays under the temp root, then ATOMIC-rename temp → dest. `--dest` MUST NOT already exist in v1 (replacement is a deferred explicit `--replace`). Assets materialize at `<dest>/<logical-path>`, so an author declaring `assets/singular/db/*.sql` yields `"$t/assets/singular/db"` for `mariachi --schema-template`.
- **Container parse** (`dmir_pkg_v0.py`): new `AssetEntry` + `_parse_assets` validate `manifest.assets` (safe path, present blob, matching length, no duplicates) into `LoadedPackage.assets`. Absent `assets` key → `[]` (the backward-compatible path: a pre-asset package loads unchanged). The loader does not reject unknown blob types, so asset blobs are additively tolerated by a code-only consumer.
- **Versioning / blast radius:** `DRIFTC_VERSION` 0.33.55 → **0.33.56**; **ABI stays 18** (package tooling + container manifest only; no runtime boundary, no calling-convention change). Container `format_version` stays `0` (additive: unknown manifest fields + blob types are tolerated). **Author-claims are unchanged** (SCI already includes asset content → source identity stable); **cert-claims must be re-issued** for asset-shipping packages because moving asset bytes inside the `.dmp` changes `artifact_sha256` (re-deploy through cert once; packages with no assets are byte-identical and unaffected). **No forced ecosystem upgrade** — code-only consumers keep loading asset-bearing packages (assets ignored); only consumers that want to extract need the new `drift unpack`. Eager-blob-read caveat noted for large assets (lazy asset-blob loading / size guard tracked as decision D-5; DB-schema packages are small and land now).
- **Review hardening (post-implementation):** (1) **TOCTOU verify→extract** — `drift unpack` recomputes `sha256(decompressed .dmp)` and requires it to equal the verified `artifact_sha256` before writing, so a `.zdmp` swapped between verify and extract is rejected (materializes nothing). (2) **Directory assets** — `manifest.resolve_asset_files` (recursive, all extensions) shared by the SCI and packing paths so they stay in lock-step; explicit symlink policy (in-root file symlink → target bytes packed as a regular file; escaping / dangling / symlink-to-directory all REJECTED, never silently skipped; unpack writes regular files only). (3) **SHA-collision cross-role guard** — an asset whose bytes collide with a code (DMIR/interface) blob is rejected at emit (driftc) AND at load (`_parse_assets`: a blob may be code or asset, never both), closing the content-addressing aliasing hole. (4) **Clean extraction failure** — extraction-side decompress/load errors are wrapped as `UnpackError` so `drift unpack --json` returns a structured exit-1 failure instead of a traceback.
- **Regressions:** `lang/tests/driver/test_unpack_cli.py` (7: happy path + consumer-flow `assets/singular/db` layout + tampered-asset fail-closed + swap-after-verify TOCTOU fail-closed + post-verify-corrupt clean failure + `--dest`-must-not-exist + no-trust-source usage error). `lang/tests/packages/test_dmir_pkg_assets.py` (8: round-trip + backward-compat "assets don't perturb module decoding" + missing-blob / len-mismatch / unsafe-path / duplicate-path / code-blob-alias / sha-collision rejection). `tools/drift_deploy/test_build.py::TestAssetDirectoryEntries` (8: directory expansion + file/dir-same-SCI + empty-dir + the four symlink cases + nonexistent passthrough). Existing package-format (78) / build+manifest (176) / verify-package (24) suites stay green. Plan/progress: `work/package-assets-zdmp/`.

## 2026-06-24 - 0.33.55: a passing `assert(cond, msg)` with a heap-built message no longer leaks the message (LANGUAGE_BUG)
### LANGUAGE_BUG: every passing `assert` whose message is a runtime-built String leaked that String
- **Symptom:** a deterministic "179 bytes in 1 block definitely lost" per process under `valgrind --leak-check=full`, allocated by `drift_string_concat` directly inside `std.concurrent::await_signal` with no application frame between `malloc` and the leak. Reported by the pushcoin team (cert run `20260624-140158-drift-lang-ab73db5`, drift 0.33.54 / abi 18): every long-running service using the standard `await_signal` → SIGINT/SIGTERM graceful-shutdown pattern failed the memcheck cert lane, blocking the whole cert pool. `await_signal` returns a `ProcessSignal` enum (no string), so the caller had nothing to free — the leak was wholly inside the stdlib/compiler.
- **Reduction (the reported `ProcessSignal::User1`/SIGUSR1 path was a red herring):** the minimal trigger is any passing assert with a non-literal message — `assert(1 == 1, "a" + "b");`. `await_signal` hit it through its single-waiter-contract diagnostic at `stdlib/std/concurrent/concurrent.drift:2334`, whose message is a three-way literal concat. A plain string-literal message never leaked (a static string's release is a no-op, `DRIFT_STRING_FLAG_STATIC`); only a heap concat/`call` result leaked.
- **Root cause (compiler lowering, not a stdlib missing-free):** `_visit_stmt_HAssert` (`lang/driftc/stage2/hir_to_mir.py`) evaluated the message expression UNCONDITIONALLY before the assert. For a heap message that produced an owned String temp which `drift_assert_loc` does not consume (the runtime takes a read-only borrow and, on the passing/`cond==true` path, returns without touching it — by contract the message was never released by the runtime). Nothing on the passing path released that owned temp → leak on every passing assert. Latent since the message-bearing-assert lowering landed; surfaced now because `await_signal` is the first common cert-lane stdlib path with a concatenated message.
- **Fix** (lowering-only): lower the diagnostic (`file`/`line`/`expr`/`msg`) and emit `AssertLoc` ONLY on the failing branch. `_visit_stmt_HAssert` now branches on the condition first (`IfTerminator` → `assert_pass` / `assert_fail`); the message is built inside `assert_fail`, immediately before an `Unreachable` terminator. The passing path never constructs the message, so there is nothing to drop and no leak; the failing path aborts, so its unreleased stake is moot (reclaimed by process death). This also makes the message lazy (built only when the assert fires) — a small bonus on the hot path. The runtime `drift_assert_loc` is now only ever reached with `cond==false` and always aborts; its comment in `lang/language_runtime/posix/assert_runtime.c` was updated to reflect that the "caller releases after the call" contract no longer applies. The sibling `_rr_abort` `AssertLoc` site (array `remove_range` bounds) already used a static-literal message on a fail-only `Unreachable` branch — unaffected.
- **Version:** `DRIFTC_VERSION` 0.33.54 → **0.33.55**; **ABI stays 18** (HIR→MIR lowering of `assert` only; no artifact-format, runtime-boundary, or calling-convention change).
- **Regressions:** `lang/tests/memcheck/test_assert_message_pass_memcheck.py` (valgrind) — a passing `assert(1 == 1, "a" + "b")` is now `definitely lost: 0`; it fails pre-fix. Hand-validated (valgrind) on the exact cert shape (`await_signal()` + delivered SIGINT → 0 errors/0 leaks), on passing concat/heap-variable/static-literal messages in one program (0 errors, no double-free of the heap-variable message), and on a FAILING concat-message assert (still prints `assertion failed: <expr> — <msg>` + location and aborts rc=134; `definitely`/`indirectly lost: 0` on the abort path). `lang/tests/driver/test_signal_await.py` (6) and `lang/tests/driver/test_assert_typecheck.py` stay green; no test pins the prior single-block assert MIR shape.

## 2026-06-20 - 0.33.46: a container `push` of a can-throw call's String result no longer double-frees at teardown; + `std.fs.read_to_bytes`/`read_to_string` (CORE_BUG + stdlib)
### CORE_BUG: `arr.push(throwing_call_returning_String())` repeated ≥2× double-freed at scope/exception teardown
- **Symptom:** a program that printed correct results aborted (SIGABRT / heap corruption → runaway `drift_alloc_array`) when a `move`-returned value built from file-read strings was torn down. Reported by DriftQuery (M3: build env from on-disk `.dbx` fixtures via `env.load_env`). Inline string literals through the SAME parse/fold/move chain were fine; only file-read (heap) strings aborted.
- **Reduction (the reported chain was a red herring):** the minimal trigger has no parser, no `move`-return, no file IO — `var a: Array<String> = []; a.push(mk(i))` where `mk` is a **`throws`** function returning a **heap** `String`, repeated ≥2×, torn down. valgrind: the buffer is alloc'd in the producer's `concat`, freed by `__drift_array_drop_String`, then read again by a stray `drift_string_release` — all in the same function. Masked for inline literals because a static string's release is a no-op (`DRIFT_STRING_FLAG_STATIC`); fatal only for heap strings.
- **Root cause:** a can-throw call's Ok payload is materialized into a hidden `__call_ok` **holder local** (`_lower_can_throw_call_value`), and the value used at the `push` arg site is a LOAD from that holder — a shared view whose stake stays owned by the holder (which the holder's own release machinery drops at every scope/exception exit). But `_call_arg_yields_owned_temp` classified the can-throw call as a free owned rvalue temp, so `_ensure_array_elem_copy(drop_source=True)` emitted `CopyValue` (retain into the array slot) **plus** `DropValue` of the shared view. The `DropValue` decremented the holder's refcount out from under it: the array slot and the holder then both referenced a buffer that the holder freed at exit (or on the next loop iteration's holder overwrite) → double free. This is the exact `Array<String>.push(local_string)` UAF shape the classifier's own docstring guards against (drift-net-tls v0.3.14), reached through the can-throw holder instead of a named variable.
- **Fix** (`lang/driftc/stage2/hir_to_mir.py`, lowering-only): centralized the rule in a shared `_expr_is_can_throw_call(expr)` helper used by BOTH container-transfer classifiers. (1) `_call_arg_yields_owned_temp` (array `push`/`insert`/`set`) returns `False` for a can-throw call arg → `drop_source=False` (retain own stake, no `DropValue`). (2) The **array-literal** element loop has its own ownership classifier with the identical `non-place ⇒ owned ⇒ DropValue` flaw — `val xs: Array<String> = [mk(0), mk(1)]` double-freed the same way; it now treats a can-throw call element as a shared view too (via the same helper). The holder's pre-existing all-edges release balances both. No type-table/ABI/runtime change.
- **Trigger registered + bounded conformance matrix.** Added a `doc/refactor_triggers.md` entry "String ownership-authoring conformance matrix" (any String-ARC leak/double-free/UAF in `string_arc.py` / cleanup authoring / ownership ledger / exception-edge cleanup / **container transfer** / **move-return teardown** escalates from one-path patch to root-cause-fix + a representative matrix). The matrix (`lang/tests/memcheck/test_string_arc_container_transfer_matrix.py`, valgrind) covers producers {heap concat, `string_from_utf8_bytes` file-read, static-literal control} × consumers {array push, **array-literal element store**, struct field, variant `Some`, local reassign} × exits {scope drop, throwing-edge unwind, move-return} + nested `Array<Shape{Array<Field{String}>}>` (the reported env shape). All consumer rows are clean under the centralized fix.
### stdlib: `std.fs.read_to_bytes` / `std.fs.read_to_string` (bundled for one certified DriftQuery release)
- Path-based whole-file helpers (opened-handle/streaming reads stay in `std.io`): `read_to_bytes(path, max_bytes, timeout) nothrow -> core.Result<Array<Byte>, io.IoError>` and `read_to_string(path, max_bytes, timeout) nothrow -> core.Result<String, io.IoError>` (spelling the error type directly, exactly as `read_dir` does). `max_bytes` is mandatory and in bytes: `<0` → `IoError(kind="invalid-argument")` (no open); `==0` accepts only an empty file; an over-cap file — **including one that grows past the cap mid-read** — fails with the distinct `IoError(kind="too-large")` rather than truncating (the cap is enforced per chunk; no stat fast-path, which would need a new runtime boundary). `read_to_string` validates UTF-8 (`std.text.utf8_from_bytes`) and returns an OWNED String; invalid UTF-8 → `IoError(kind="invalid-utf8")`. Error precedence: read / decode / too-large win over a close error; a close-only failure returns the close error. Timeout/cancel/saturation surface via the underlying `io` read, consistent with `read_dir`; no implicit unbounded wait. Implemented purely over existing public `std.io`/`std.text` APIs → **no new runtime symbols**. Added `IO_ERROR_KIND_INVALID_ARGUMENT` / `IO_ERROR_KIND_TOO_LARGE` to `std.io`. (A convenience `pub type Result<T> = core.Result<T, IoError>` alias was prototyped but REMOVED: a partially-applied generic type alias in `std.io`'s exported surface evaluates to `UNKNOWN` during `--emit-package` interface-schema validation, breaking package emit for ANY package compiled alongside `std.io` — it failed the mariadb-rpc cert. The helpers spell `core.Result<…, io.IoError>` directly instead. The underlying alias-evaluation defect is a separate latent compiler bug, not fixed here.) Tests: `lang/tests/driver/test_std_fs_read_to.py` (contract + ownership-move valgrind).
- **Version:** `DRIFTC_VERSION` 0.33.45 → **0.33.46**; **ABI stays 17** (lowering classifier fix + stdlib over existing APIs; no runtime-boundary change).
- **Regressions:** seed + matrix (memcheck, 9) all valgrind-clean and the seed fails pre-fix (`new_temp`-revert-independent; reverting the classifier fix reproduces the double-free); `std.fs` contract + ownership (3). The original DriftQuery-shape repros (records/enums + schema + artifact-signature fixtures through `load_env` move-return) are valgrind-clean post-fix. Existing array/string memcheck suites (incl. the drift-net-tls `push(local_string)` guard) stay green.

## 2026-06-20 - 0.33.45: compiler temporaries use a non-source-expressible (`.t<N>`) namespace so a user variable named `t1`/`t2` no longer collides with a same-named temp (CORE_BUG)
### CORE_BUG: a source variable named like a compiler temp (`t1`/`t2`/`t<N>`) corrupted codegen ("phi with mixed incoming types {ptr, drift.int}")
- **Symptom:** a program that typechecks aborted during LLVM lowering with `NotImplementedError: LLVM codegen v1: phi with mixed incoming types {'ptr', 'drift.int'}` (`_lower_phi`), or other codegen corruption (e.g. `instruction forward referenced with type 'ptr'`). Reported by DriftQuery (M3.3 fixture loader): adding an artifact-folding branch that declared `var t1 = -1; var t2 = -1;` made EVERY consumer of `dqc.env` fail to codegen. The reporter could not minimize it to a standalone file — the trigger looked size/shape-sensitive.
- **Root cause (name-collision between the compiler temp namespace and user identifiers):** `MirBuilder.new_temp()` allocated intermediate-value names as bare `t<N>` (`t1`, `t2`, `t3`, ...). A user source variable literally named `t1`/`t2`/`t68` has the SAME base name as a compiler temp. `func.local_types` is keyed by base name, so both the user local and the same-named temp wrote `local_types['t2']`; when the temp carried a reference/pointer type — e.g. `t2 = AssignSSA(e_1)`, the receiver copy of a `&mut Env` param passed to an early call — its `&mut`/`ptr` type clobbered the user variable's `Int` type. When the user `t2` was declared inside a loop (so SSA inserts an entry-default `ZeroValue` whose type is read from `local_types['t2']`), the corrupted `ptr` type flowed into the entry edge of the loop-header phi while the real assignments supplied `drift.int` on the other edge → a phi with mixed `ptr`/`int` incomings.
- **Why not a `__` prefix:** other compiler-internal names commonly use a leading double-underscore (`__borrow_tmp`, `__array_cap_grew`, `__recv`, ...) and the grammar comment *describes* `__` as compiler-reserved, but the grammar's `NAME` rule does NOT actually reserve it — `var __t1 = ...` parses today, and stdlib even exports `__test_*` hooks. So renaming temps to `__t<N>` would only move the same collision to a user `__t<N>`. The fix instead uses a character source cannot produce.
- **Fix** (lowering + codegen, no ABI/runtime change):
  - `lang/driftc/stage2/hir_to_mir.py` — `new_temp()` returns `.t<N>` (leading `.`). The grammar's identifier rule draws only from `[A-Za-z0-9_]`, so no source variable/parameter/binder — present or future — can equal `.t<N>` or any compiler name that embeds it (several sites build addr-taken locals like `__replace_old_<new_temp()>`). `.` is a legal LLVM identifier char, emitted as `%.t<N>`.
  - `lang/codegen/llvm/llvm_codegen.py` `_alloca_name_for_local` — now preserves every char legal in an unquoted LLVM identifier (`[A-Za-z0-9_$.]`) instead of collapsing non-alphanumerics to `_`. This is a no-op for all source-originated names (already `[A-Za-z0-9_]`) but is load-bearing: it stops an addr-taken temp like `.t5` from being sanitized onto `_t5` and colliding in the alloca-pointer namespace with a user `_t5`.
  - `lang/codegen/llvm/llvm_codegen.py` `_bb` — the sibling block-label helper had the same false `__bb_` "reserved" assumption (a user local `__bb_entry` could collide with a block label, since labels and SSA values share one LLVM namespace). Switched to a non-source `.bb.<name>` prefix. (Codegen-internal SSA names minted by `_fresh`, including its sub-block labels, remain in the source-collidable `[A-Za-z0-9_]` space — a separate, lower-severity latent issue: it needs an exact accidental name match and fails *loudly* as an LLVM verifier duplicate-name error, never a silent miscompile. Tracked for a focused follow-up; not addressed here.)
  - The phi abort itself is NOT changed: `_lower_phi`'s mixed-type guard correctly refused to emit malformed IR; the bug was the upstream mis-typed local.
- **Version:** `DRIFTC_VERSION` 0.33.44 → **0.33.45**; **ABI stays 17** (HIR→MIR temp naming + codegen name-emission only).
- **Regressions:** stage2 unit `test_mir_temp_name_reserved_namespace.py` (3) pins the invariant grammar-independently — every `new_temp()` output contains a character outside the source identifier charset `[A-Za-z0-9_]` (so it cannot equal any source identifier), is the chosen `.t<N>` form, and never equals `t1`/`t2`/`t68`/`t163`/`__t1`/`__t2`/`_t1`/`_t5`. Driver `test_temp_user_name_collision_codegen.py` (4) is the minimal standalone reproduction the original report could not isolate — an early call passing a `&mut` param (producing a ptr temp that lands on `t2`) plus user `Int` sentinels declared inside an outer loop and used as post-loop array indices — parametrized over `t1`/`t2` (original bug), `__t1`/`__t2` (proves it is not a `__` fix), and `_t1`/`_t2` (exercises the alloca-name path); each compiles/runs and returns `80`. A fourth case names locals `__bb_entry`/`__bb_if_then` (the `_bb` hardening) and returns `18`. Pre-fix the `t1`/`t2` form fails to codegen (verified by reverting `new_temp` to `t<N>`). Full stage2/stage3/stage4 + codegen + checker unit suites (406) and the driver suite stay green.

## 2026-06-19 - 0.33.44: explicit-deref place borrows (`&(*p)`, `&p->children[i]`) now shallow-type correctly instead of degrading to Unknown (checker cleanup)
### Checker cleanup: shallow place walker now types explicit `HPlaceDeref` projections (follow-up to 0.33.43)
- **Context:** the 0.33.43 fix added an `HPlaceIndex` arm to the shallow place walker (`Checker._TypingContext._infer_expr_type`, the `HPlaceExpr` branch) but deliberately left explicit `HPlaceDeref` projections bailing to `None`, because the loop did an **unconditional leading-REF unwrap before every projection** — a naive deref arm would double-peel `&&T` (→ `T` instead of `&T`) and reject `*p` for `p: &T`. This slice removes that hazard and types deref places exactly.
- **Symptom (latent, now fixed):** a borrowed local taken through an EXPLICIT deref *place* — `val head = &(*p);` or `val head = &(*p).children[0];` (and the `->` sugar `&p->children[0]`, normalized to `(*p).children[0]`) — degraded to `Unknown`, so a `String` field read off the borrowed local typed as `Unknown` and the SECOND string op on the derived local mis-fired `string binary ops require String operands … [E-AUTO-f6706407]` (the same cascade as 0.33.43, reached via `HPlaceDeref` instead of `HPlaceIndex`). Plain value reads `p->text` / `(*p).text` were never affected — they go through the `HUnary(DEREF)`+`HField` path, not the `HPlaceExpr` place walker.
- **Key model finding:** the **HIR** `HPlaceExpr` (built structurally by `stage1/place_expr.py`, no type info) uses an *implicit* convention — `HField`/`HIndex` projections carry an implicit one-level auto-deref of a ref base; an explicit `HPlaceDeref` is emitted only from a source unary deref (`*`, and `->` normalized to `*`). This is a different convention from the **MIR**-level `Place` typed by `borrow_checker_pass._type_of_place` (explicit derefs, no implicit unwrap) — the two layers must not be conflated, so the MIR walker is *not* the model to mirror here.
- **Fix** (`lang/driftc/checker/__init__.py`, checker-only): restructured the projection loop. Removed the unconditional leading-REF unwrap; the implicit single-level unwrap now lives **inside** the `HPlaceField` and `HPlaceIndex` arms (`_peel_ref_once` → struct/array host), and the new `HPlaceDeref` arm peels **exactly one** reference level with no pre-unwrap (`*p` on `&T` → `T`, on `&&T` → `&T`). Single-level (not a loop) matches the language's single-level auto-deref; `TypeKind.REF` covers both `&T` and `&mut T` (mutability is a `ref_mut` bool, no `REF_MUT` kind), so `&mut` needs no special case. Place projections return the field/element TYPE only and (like the 0.33.43 index arm) never run a Copy check — a place projection is a borrow, not a copy. No MIR/lowering/runtime/ABI change.
- **Version:** `DRIFTC_VERSION` 0.33.43 → **0.33.44**; **ABI stays 17** (checker shallow-inference cleanup only).
- **Regressions:** checker unit `test_place_deref_shallow_inference.py` (9) pins inferred `TypeId`s directly via `_TypingContext.infer` — `*p`(&T)→T, `*pm`(&mut T)→T, `*pp`(&&T)→**&T** (the exact one-level peel the old leading-unwrap got wrong), `(*p).text`→String, `(*pp).text`→String, `(*p).children[0]`→Node, ref-array index, ref-struct field, plus a non-Copy-payload case asserting **no diagnostic** (no Copy false accept/reject). Driver `test_borrowed_local_deref_field_string_copy.py` (3) compiles+runs+leak-clean for `&(*p)`, `&(*p).children[0]`, and `&p->children[0]` with the borrowed String field reused in a second string op. Pre-fix emulation (deref arm forced to bail) reproduces `E-AUTO-f6706407` at both deref cascades; post-fix both are clean. Full checker+stage1+type_checker (287) and the string/array/borrow/field/index/place/deref driver set (381) stay green, as does the 0.33.43 `test_borrowed_local_field_string_copy.py`. Design/rationale: `work/place-deref-shallow-infer-cleanup.md`.

## 2026-06-19 - 0.33.43: a borrowed array-element local (`&arr[i]`) no longer types as Unknown and mis-fires the string-operand diagnostic (LANGUAGE_BUG)
### LANGUAGE_BUG: `val h = &arr[i]; val a = h.field + ""; val b = a + ""` falsely rejected the second concat
- **Symptom:** taking a shared borrow of an array child and reading a `String` field off it — `val head = &pl.children[0]; val name = head.text + ""; val again = name + "";` — raised `error: string binary ops require String operands and support only +, ==, !=, <, <=, >, >= [E-AUTO-f6706407]` at the **second** concat (`name + ""`), on valid source. The asymmetry hid it from earlier repros: `val name = head.text + ""` alone compiled, and `take_name(head.text)` (param expects `String`) compiled — the defect only surfaced once the binding was reused in another string op. Reported against a `struct Node(text: String, children: Array<Node>)` walked through a borrowed child.
- **Root cause (shallow checker inference):** `Checker._TypingContext._infer_expr_type` (the stub/shallow inference in `lang/driftc/checker/__init__.py`) lowers an addressable place `HPlaceExpr` by walking its `projections`, but the loop handled only `HPlaceField` — every other projection kind fell through to `else: return None`. `&pl.children[0]` normalizes to a place `base=pl, projections=[Field("children"), Index(0)]`; the `Index(0)` is an **`HPlaceIndex`** projection, so the walk returned `None` and `val head = …` was recorded as **`Unknown`** (the `decl_ty or value_ty or _unknown_type` fallback). The cascade: `head.text` (subject `Unknown`) inferred `None`; `head.text + ""` (left `None`) returned `None`, so `name` was recorded `Unknown`; finally `name + ""` saw left = a **known** non-String (`Unknown`, not `None`) and tripped the `non_string_known` branch → the spurious diagnostic. Same parallel-walker-omission class as the 0.33.36 match-arm renamer and 0.33.37 cast-borrow canonicalizer (a node kind a walker forgot to handle).
- **Fix** (`lang/driftc/checker/__init__.py`, checker-only): added an `HPlaceIndex` arm to the place-projection loop — on an `ARRAY` `cur_def`, project to the element type `param_types[0]` (the existing top-of-loop leading-REF unwrap already peels `&Array<T>` to `Array<T>`). Critically this is a **place** projection (a borrow of the element), so unlike the value-context `HIndex` branch it does **not** emit a Copy diagnostic for non-Copy element types. Explicit `HPlaceDeref` projections are deliberately left bailing to `None` (their pre-existing behavior): the unconditional leading-REF unwrap is correct for field/index projections but would double-peel an explicit deref (`&&T` → `T` instead of `&T`), so typing them is out of scope for this fix. No type-table, MIR, ABI, or runtime-boundary change.
- **Version:** `DRIFTC_VERSION` 0.33.42 → **0.33.43**; **ABI stays 17** (checker shallow-inference fix only).
- **Regressions:** driver `test_borrowed_local_field_string_copy.py` (2) — the borrowed-array-child field-concat shape and the same shape behind a `&Node` parameter (with `take_name(String)` slot passing) both compile, run, and are leak-clean. Fails before the fix (`E-AUTO-f6706407`). Validated checker (44) + stage1 (131 combined) and 371 array/string/borrow/field/index/place driver tests stay green.

## 2026-06-17 - 0.33.39: a `match` arm returning `move <local>` no longer drops the value and yields a zeroed variant (CORE_BUG)
### CORE_BUG: a `match` arm whose trailing result `move`s an arm-body local mis-lowered to `Ok(None)` + a use-after-free
- **Symptom:** an arm `pattern => { val out = f(); move out }` (the trailing `move out` consuming a local declared in the SAME arm body) yielded a ZEROED variant instead of the moved value, and freed the moved value's heap payload. For a nested variant result `Result<Optional<Array<Byte>>, _>` the zeroed storage decodes as `Ok(None)`, so a present value was observed as absent (a latent use-after-free of the dropped payload as well). Reported by drift-query (lmdb-engine-poc) as **BUG-2**: a `Storage.get` of a present key returned `Some` internally but the caller observed `None`, through a `with_read_txn<R>(store, body)` driver whose `Ok(txn) => { val out = body.call(&txn); store.end_read(&txn); move out }` arm returned `move out`. The report attributed it to generics + dynamic dispatch + prior mutation of the boxed `Storage`; **reduction proved all of those are irrelevant** — the minimal trigger is a plain single-module `match` arm returning `move <local>` whose value type has a droppable payload (no generics, no interface, no callback, no Mutex, no prior write). Deterministic (the report's "debug print perturbs it" was the same underlying instability, here fully deterministic per source).
- **Root cause (HIR→MIR scope nesting):** a `match` arm `pattern => { stmts; result }` is a single block expression — the trailing `arm.result` shares the arm body's lexical scope and may CONSUME a local declared in `arm.block`. `_lower_match` lowered the arm body via `self.lower_block(arm.block)`, which pushes its OWN nested scope, lowers the statements, and emits that scope's cleanup `M.CleanupHook` (candidate = the body local `out`) — **before** `arm.result` (`move out`) was lowered further below. So in the raw MIR the order was `StoreLocal(out, …)` → `CleanupHook([out])` → `MoveOut(out)`. `cleanup_authoring`'s ledger walks in program order: at the `CleanupHook` `out` is still LIVE (its `MoveOut` is later), so it authored an unconditional drop — running the destructor AND zeroing `out` — and the subsequent `MoveOut` then read the now-zeroed slot. The arm scope (pushed per-arm) and the `lower_block` scope were redundantly nested and coextensive for a match arm; only the inner one closed too early. (The `Err` arm was unaffected: its result is a fresh `Result::Err(e)` construction, not a `move` of a body local.)
- **Fix** (`lang/driftc/stage2/hir_to_mir.py`, lowering-only): in `_lower_match`, lower the arm body statements DIRECTLY into the arm scope (a bounded `for stmt in arm.block.statements: lower_stmt(stmt)` with the standard terminator guard) instead of through `lower_block`'s nested scope. The body local now lives in the arm scope and its cleanup defers to the arm-end `M.CleanupHook` (already emitted AFTER `arm.result`), so the ledger sees the result's `MoveOut` first and correctly skips the consumed local. Block-local lifetimes are unchanged (the nested scope was coextensive with the arm scope); nested blocks/ifs/loops inside the body still get their own scopes via `lower_stmt`. No checker, ABI, or runtime-boundary change.
- **Version:** `DRIFTC_VERSION` 0.33.38 → **0.33.39**; **ABI stays 17** (HIR→MIR lowering fix only).
- **Regressions:** driver `test_match_arm_move_local_result_scope.py` (3) — `Ok(t) => { val out = get(0); move out }` yields the real nested-variant value (returns `7`, was `Ok(None)` → `2`); the same shape with a droppable `String` payload survives and is leak/UAF-clean (returns `5`, was `99`); and the direct-call control arm (`Ok(t) => { get(0) }`, never affected) still works. Each `move`-local case fails before the fix and the controls pass; the repro is valgrind-clean. The drift-query lmdb-engine-poc `mem_store_test` now reports ALL PASS (was `FAIL: get k2 missing`). Full stage2 suite (287) stays green.

## 2026-06-16 - 0.33.38: discarded `Array<T>.pop()` / `.remove()` / `.swap_remove()` results no longer drop the array mutation (CORE_BUG)
### CORE_BUG: a value-returning array mutator was a silent no-op when its result was discarded
- **Symptom:** `a.pop();` in statement position (the returned `Optional<T>` not bound) compiled and "succeeded" but **did not remove any element** — `var a = [10,20,30]; a.pop(); a.len()` stayed `3`. Same for discarded `a.remove(i);` and `a.swap_remove(i);`. The app team reported `Array<T>.pop()` as "an unsupported method that is being erroneously accepted as a no-op": it must either work or fail to compile. Binding the result (`val x = a.pop()`) always worked correctly (len `3→2`, drain loops drained), as did the Void-returning mutators `push`/`insert`/`truncate`/`swap` when discarded — only the value-returning mutators were affected, and only on the discard path.
- **Root cause:** the HIR→MIR statement-discard path (`_visit_stmt_HExprStmt`) lowers an array-intrinsic method call with `want_value=False`. The `pop` and `remove`/`swap_remove` lowerings in `_lower_array_intrinsic_method` opened with `if not want_value: return True, None` — returning `handled=True` (so the call was accepted and nothing else ran) **before** emitting the `ArraySetLen`/`StoreRef` that performs the actual mutation. So a discarded value-returning mutator removed the element from the SSA shape conceptually but never stored the shortened array back through the receiver pointer. The early bail was a mis-applied copy of the read-only-accessor pattern (`len`/`cap`/`gen`/`get`/`ref_at`/`range`, where a discarded result genuinely is a no-op) onto methods that mutate.
- **Fix** (`lang/driftc/stage2/hir_to_mir.py`, lowering-only): removed the early `if not want_value: return True, None` from `pop` and `remove`/`swap_remove`. The mutation now always runs; only the `Optional<T>` result staging is skipped on discard (`pop`'s `res_local`/`Some`/`None` machinery is gated on `want_value`; `remove`/`swap_remove` skip the final `return True, val`). On the discard path the element is moved out of the array (`ArrayElemTake`) and is no longer owned by it, so it is **dropped** (`M.DropValue`, guarded by `_needs_runtime_drop`/`_type_is_destructible`) — its destructor still runs, no leak. The read-only accessors keep their `if not want_value: return True, None` (correctly a no-op). No checker, ABI, or runtime-boundary change.
- **Version:** `DRIFTC_VERSION` 0.33.37 → **0.33.38**; **ABI stays 17** (HIR→MIR lowering fix only).
- **Regressions:** driver `test_array_discarded_mutator_result.py` (5) — discarded `pop`/`remove`/`swap_remove` each shorten the array by one; repeated discarded `pop` drains; a discarded `pop` of a `String` element runs and is leak-clean (lane-audit `leaks=0`). All fail before the fix (len unchanged). The bound-result paths are unchanged: e2e `array_pop_move_out_non_copy`, `array_pop_move_out_string`, `array_swap_remove_basic`, `generic_array_pop_optional_typevar`, `variant_internal_tombstone_array_pop`, `std_array_remove_range`, `array_borrow_tmp_drop`, and driver `test_array_method_contract.py` all stay green.

## 2026-06-16 - 0.33.37: `&local` borrow nested in a `cast` no longer double-frees + same-width narrow-int comparisons (LANGUAGE_BUGs)
### LANGUAGE_BUG: a `&local` borrow nested inside `cast<…>(…)` double-freed the local
- **Symptom:** `cast<mem.Ptr<Uint>>(io.buffer_ptr(&b))` — where `b` is a `var` Buffer (owns a heap allocation, has a destructor) — double-freed `b` at scope exit (`free(): double free detected in tcache 2`, exit 134). Reported by drift-query (lmdb-engine-poc) against an FFI buffer; reduced to a single inline cast-of-borrow-bearing-call with no FFI. Binding the pointer to a `val` first was the reporter's workaround and was always fine.
- **Root cause:** the stage-1 place-canonicalizer (`place_canonicalize._rewrite_expr`) and borrow-materializer (`borrow_materialize._rewrite_expr`) had **no `HCast` case** — neither recursed into a cast operand. So a borrow nested inside a cast kept a bare `HVar` subject instead of being canonicalized to an `HPlaceExpr`. At HIR→MIR, `_visit_expr_HBorrow` saw a non-`HPlaceExpr` subject, fell into the **rvalue-borrow fallback**, and materialized the named local into a `__borrow_tmp` (registering a drop) — so the Buffer was dropped once as the borrow-temp cleanup and once as the named local. (Confirmed in the emitted IR: two `Buffer::destroy` calls on the same value; the OK control had one.) Same walker-coverage bug class as the 0.33.36 match-arm renamer.
- **Fix:** added `HCast` (and `HResultOk`) cases to both `place_canonicalize._rewrite_expr` and `borrow_materialize._rewrite_expr` so they recurse into the operand. Pure stage-1; no type-checker/runtime change. Repros run clean and valgrind-clean.
### LANGUAGE_BUG: same-width narrow-int comparisons crashed codegen
- **Symptom:** `Uint32 == Uint32` (and other same-type narrow-int comparisons) raised an internal Python traceback at codegen: `NotImplementedError: LLVM codegen v1: integer binop requires matching Int/Uint operands (have i32, i32)`.
- **Root cause:** `llvm_codegen._lower_binary`'s integer-op `int_ty` selection recognised only `Int`/`Uint`/`Uint64`/`Byte` operands, never `i32` (`Int32`/`Uint32`).
- **Fix:** added an `i32` branch gated to the six comparison ops (`EQ/NE/LT/LE/GT/GE`) → lowers to `icmp` producing `Bool`, with **correct signedness**. The LLVM `i32` type does not encode signedness (and there is no value→TypeId map at codegen), so signedness is carried on the instruction: `BinaryOpInstr` gained a `signed: Optional[bool]` field, set by HIR→MIR (`_visit_expr_HBinary`) from the operand's drift type (`Int32`→True, `Uint32`→False). Codegen selects unsigned `icmp u*` for `Uint32` and signed `icmp s*` for `Int32` (EQ/NE are signedness-agnostic regardless). So a high-bit `Uint32` (`0xFFFFFFFF`) correctly compares **greater** than `1`, not less. Narrow-int **arithmetic** stays out of scope — the `int_ty is None` raise still rejects `i32` add/sub/etc. (the marker-based "preserve signedness in value_types" alternative was rejected: it would touch every `"i32"` site across both builder classes; threading `signed` on the comparison instruction is the contained central representation).
- **Version:** `DRIFTC_VERSION` 0.33.36 → **0.33.37**; **ABI stays 17** (stage-1 + codegen fixes; no runtime-boundary change).
- **Regressions:** driver `test_borrow_in_cast_no_double_free.py` (3) — cast-of-borrow-bearing-call runs clean, the reporter's nested-cast chain runs clean, and the val-bound control still works; `test_narrow_int_comparison.py` (5) — `Uint32 ==`, `Int32 ==`, `Int32` signed ordering, `Uint32` UNSIGNED high-bit ordering (`cast<Uint32>(4294967295u) > cast<Uint32>(1u)` is true), and the inverse `<` case. Both crash/abort before their fixes; the high-bit cases are silently wrong under signed-best-effort.

## 2026-06-15 - 0.33.36: `match`-arm pattern bindings are now visible inside nested `try`/`catch` and bare `{ }` blocks (LANGUAGE_BUG)
### LANGUAGE_BUG: a match-arm binding was `unknown name` inside a nested try/catch or bare block
- **Symptom:** a binding introduced by a `match` arm pattern (`core.Result::Ok(s) => { … }`) was **not visible** inside a nested statement-form `try { … } catch … { … }` or a nested bare `{ … }` block within that arm — `error: unknown name 's' [E-AUTO-18502502]` at every use. The SAME binding was visible in the arm's direct statements and inside nested `if` / `while` / `match` bodies, and a plain `val` local at the same lexical position was visible everywhere. So the defect tracked the *kind* of child block, and only for pattern-introduced bindings.
- **Root cause:** match-arm binders are scoped by **alpha-renaming** in `ast_to_hir._lower_match_expr` — `_rename_stmt`/`_rename_expr`/`_rename_block` walk the arm body and rename binder USES to a unique internal name (HIR locals are function-scoped). `_rename_stmt` had explicit recursion for `HLet`/`HAssign`/`HReturn`/`HThrow`/`HExprStmt`/`HIf`/`HLoop`, but **fell through (returned the statement unchanged) for `HTry`, `HBlock`, `HUnsafeBlock`, and `HAssert`** — so binder uses inside a nested try/catch, bare block, unsafe block, or an `assert(cond, msg)` were never renamed, kept the original source name, and failed to resolve. `if`/`while` worked only because `HIf`/`HLoop` were handled; plain `val` locals were unaffected (lexical scope, no renaming). Separately, the **expression-form** `HTryExpr` renamer applied the outer mapping unchanged inside catch arms, so a catch binder shadowing the match-arm name was incorrectly rewritten toward the match binder.
- **Fix** (`lang/driftc/stage1/ast_to_hir.py`, pure stage-1): added `HTry` / `HBlock` / `HUnsafeBlock` / `HAssert` cases to `_rename_stmt` (`HTry` renames the try body + every catch handler; `HAssert` renames both `cond` and `msg`), and applied the catch-binder **shadow-map removal** to BOTH statement-form `HTry` and expression-form `HTryExpr` (a catch binder that shadows the match-arm name is dropped from the rename map for that handler, so the catch binder wins). Nested-block statements return the outer mapping unchanged (nested `let`s don't leak). `HRethrow`/`HLocalConst`/`HBreak`/`HContinue` carry no renamable expressions, so the statement coverage is now complete. No type-checker, resolver, or runtime-boundary change.
- **Version:** `DRIFTC_VERSION` 0.33.35 → **0.33.36**; **ABI stays 17**.
- **Regressions:** driver `test_match_arm_binding_nested_block_scope.py` (7) — match-arm binder visible inside try/catch (body + handler), bare block, try-nested-in-bare-block, and an `assert(cond, msg)`; the `if`/`while` cases still resolve (guard for the explicitly-handled kinds); and a catch binder correctly shadows the match-arm binder in BOTH statement-form and expression-form try/catch. Each failing case fails before its fix (`unknown name`, or the shadowed `s.tag` resolving to the wrong binder → Unknown receiver); the controls pass. (The reporter's exact repro additionally trips an orthogonal, pre-existing behavior — a typed `catch E(e)` does not discharge a `nothrow` throw-effect, only a catch-all `catch { }` does — so the regressions use a throwing helper wrapped by a catch-all `main` to isolate binder-scope resolution.)

## 2026-06-14 - 0.33.35: typed-catch binder fed into a native variant-constructor field no longer miscompiles into a double-free SIGSEGV (LANGUAGE_BUG)
### LANGUAGE_BUG: `catch E(e) { Result<T,E>::Err(move e) }` crashed (exit 139) on unwind
- **Symptom:** a typed `catch ErrType(e)` clause that binds the error and passes the WHOLE binder into a native variant-constructor payload field — `return Result<T, E>::Err(move e)` (or `Err(e)`) where `E` has a droppable field (e.g. `String`) — compiled cleanly but SIGSEGV'd (rc=139) on the error-unwind path. The app report attributed it to recursive value types / mid-construction; **reduction proved recursion is IRRELEVANT** — the minimal trigger is a typed catch + `Err(move e)` + a droppable error field.
- **Root cause (first broken contract):** a typed-catch binder `e` has canonical type `Error` — a read-only projection *view* over the in-flight error envelope, never the native struct `E`. `Result<T,E>::Err(...)` expects the Path-A native struct `E`. The qualified variant-constructor path in `call_resolver.resolve_call_expr` accepted whatever `instantiate_sig` unified (loose) and applied only the `&T→T` rewrite, so the `Error → E` mismatch passed the checker. LLVM's `ConstructVariant` then **autoloaded** a struct value out of the `Error` view pointer (its broad pointer-autoload fired for ANY pointer-typed arg), so the `String` inside the view and the reconstructed payload aliased one allocation → a double `drift_string_release` → use-after-free → SIGSEGV. This was NOT a string_arc/retain-drop defect; MIR had coherent HMove/MoveOut and cleanup. Two contract layers were broken: the checker accepted a type mismatch, and codegen masked it.
- **Fix, part 1 — strict constructor-boundary validation (checker).** In the qualified variant-constructor path, *after* the existing `&T→T` rewrite and *before* `record_call_info`, every argument is now validated against its INSTANTIATED field type (mapped via `ctor_arg_field_indices`) using the struct constructor's canonical type-equivalence logic — lifted to **file-level helpers** `_ctor_same_type` / `_ctor_dealias_zero_param` and reused by both constructors (the struct path's local closures now delegate; no struct behavior change). `instantiate_sig` is unchanged (validation lives at the call site, not in inference/substitution). The gate is **strict**: callback-wrap and concrete→interface boxing are NOT mirrored (an exact interface-typed value matches by type and is accepted; a concrete implementer into an interface payload is rejected — it was never a recorded variant coercion), and the only non-equivalence acceptance is the narrow unsafe `Ptr<T>→&T` coercion below. The strict boundary exposed **two pre-existing, independent producer LANGUAGE_BUGs** (initially papered over with broad gate tolerances that were then removed in favor of fixing the producers):
  - **Producer A — generic struct field lowering.** `resolve_variant_ctor._lower_generic_expr` instantiated generic VARIANT field types but returned the bare base for generic STRUCT fields, so a field declared `Pair<Int, String>` (or json's `HashMapIter<String, JsonNode, DefaultBuildHasher>`) collapsed to `Pair<Unknown, Unknown>`. Added the symmetric struct branch: `ensure_struct_template` when an arg carries a typevar, else `ensure_struct_instantiated`. (Reduced to a 4-line `Pair<Int,String>` repro with no `iter()` — the argument was always concrete; the FIELD type was wrong, contrary to the initial "`obj.iter()` loses substitutions" hypothesis.)
  - **Producer B — qualified-generic argument scoping.** `resolve_opaque_type` resolved CALLER-SUPPLIED type arguments in the OUTER nominal's module, so `mem.Ptr<S>` written in module `t` produced a pointer to a phantom `std.mem.S` instead of `t.S` (and `cast<mem.Ptr<LoggerRuntimeState>>` in std.log produced `Ptr<std.mem.LoggerRuntimeState>` vs the field's `&std.log.LoggerRuntimeState`). Fixed centrally with `arg_module = module_id if module_id is not None else origin_mod`: the outer nominal and alias BODIES still resolve in `origin_mod`, but unqualified supplied arguments resolve at the use site (`module_id`), and an explicitly-qualified argument keeps its own module — applied consistently across the Ptr/RawPtr/ref/alias-arg/generic-nominal/FnResult/fn/Array branches.
  - **Producer C — bare-name local-nominal precedence vs a re-export alias.** `std.core` re-exports `core.Box<T>` via `export { std.core.box.* }`, registering `Box` as a UNIQUE cross-module alias. The generic-expression resolvers consulted `find_unique_type_alias_by_name` whenever the *origin module* had no exact alias of that name — *before* checking for an exact local nominal — so a user module declaring its own `struct Box` / `variant Box<T>` had every bare `Box` hijacked by the re-exported `core.Box` (a spurious `have Box, expected Box` constructor mismatch, or an `Unknown` return type that ICE'd at MIR lowering). Fixed by enforcing the precedence *type param > exact local alias > exact local nominal > re-export alias > unique nominal > forward/unknown*: for an UNQUALIFIED name, the unique cross-module alias fallback is consulted ONLY when `origin_mod` declares neither an exact alias nor an exact struct/variant/interface nominal of that name; explicitly-qualified names resolve through their qualifier only. Applied at every `find_unique_type_alias_by_name` site — `call_resolver._lower_generic_expr`, `type_checker._lower_generic_expr`, `TypeTable._eval_generic_type_expr` (`resolve_opaque_type` has no such fallback). No shared helper (the three sites keep their own diagnostic behavior). This is the defect surfaced by the strict gate against the (separately uncommitted) core.Box feature.
- **Fix, part 1b — narrow unsafe `Ptr<T>→&T` / `Ptr<T>→&mut T` coercion.** After Producer B the std.log pointees are identical, so the blanket pointer-class tolerance is replaced by a precise coercion: permitted ONLY when the canonical pointees are identical AND in an unsafe context, recorded in checked HIR as an explicit `ptr_to_ref_coercions` mark (NOT type equivalence). The mark is threaded into `HIRToMIR` parallel to `iface_coercions` and **consumed in `lower_expr`**: the source pointer is lowered, the contract is re-asserted (RAW_PTR source, REF target, identical canonical pointees), and the value is re-bound via `AssignSSA` as a reference-typed SSA value — explicit downstream, no representation change (both are `ptr` values). Rejects `&T→&mut T`, `Ptr<A>→&B`, and unrelated refs. Both the checker's pointee acceptance and the lowering's pointee assertion use the **same** strict canonical-identity helper `_ctor_canonical_identity` (zero-param alias dealiasing + exact TypeId match + normalized TypeKey equality, package context derived from the type table exactly as the checker derives it) — NOT raw TypeId equality (which would spuriously assert at lowering on an alias/normalization case that type-checks) and NOT the broader `_ctor_same_type` (whose same-named-typevar and nothrow→throwing-fn subtyping carve-outs are valid *assignment* relaxations but unsafe for pointer *identity*, especially `&mut T` — a `Ptr<fn() nothrow>` is not the same referent as a `&fn()`). `_ctor_same_type` keeps those carve-outs for ordinary constructor-argument validation.
- **Fix, part 2 — provenance-specialized diagnostic.** A rejected argument proven to be a whole-value use (no field projection) of a CURRENT-function typed-catch binder whose type is the `Error` view → `E_TYPED_CATCH_BINDER_NOT_VALUE` (advises field reconstruction); any other payload mismatch → ordinary `E_VARIANT_CTOR_ARG_TYPE`. `_typed_catch_binders` is now **function-scoped** (reset at `check_function` entry): binding ids reset per function, so the previously-accumulating dict could let one function's catch-binder id collide with an unrelated binding in the next and falsely tag it (this exact collision cascaded false ctor mismatches into stdlib during development). The current function's binder ids are threaded into `CallResolverContext.typed_catch_binder_ids` (new defaulted field, populated at the three `make_call_ctx` sites).
- **Fix, part 3 — narrowed LLVM autoload (defense in depth).** `ConstructVariant`'s payload autoload (load-value-from-pointer on LLVM type mismatch) is now permitted ONLY for pointers proven to originate from `VariantGetFieldAddr` (the borrowed-match reconstruct `match v { V::N(n) => V::N(n) }`), tracked per function in `variant_field_addr_ptrs`. An arbitrary address-producing value reaching that slot raises an internal lowering-contract failure instead of being silently autoloaded.
- **Supported remedy:** reconstruct the native error from the binder's fields — `catch E(e) { return Result<T,E>::Err(E(msg = e.msg)) }` (compiles, runs, valgrind/ASan-clean). `Result<T, Error>::Err(move e)` (field type IS `Error`) remains valid — the binder type matches.
- **Version:** `DRIFTC_VERSION` 0.33.34 → **0.33.35**; **ABI stays 17** (checker + codegen-guard correction; no runtime boundary signature, layout, or calling-convention change).
- **Regressions:** driver `test_typed_catch_binder_native_ctor_field.py` (8) — `Err(move e)`/`Err(e)` rejected with code/phase/severity/span; reconstruct-from-fields compiles+runs; `Result<T,Error>::Err(move e)` valid; `&String→String` payload still accepted+runs; exact-interface payload accepted; concrete→interface rejected with the ORDINARY diagnostic; cross-function binding-id reuse does not inherit catch provenance. Codegen `test_construct_variant_autoload_provenance.py` (2) — arbitrary `AddrOfLocal` pointer into ConstructVariant raises the contract failure; a `VariantGetFieldAddr` pointer still autoloads. Producer A + the Ptr coercion: driver `test_variant_ctor_generic_field_and_ptr_coercion.py` (7) — generic struct field matches its concrete arg and REJECTS a mismatched `Pair<Int,Bool>` (structural pin that the field is exactly `Pair<Int,String>`, not `Pair<Unknown,Unknown>`); unsafe `Ptr<S>→&S` compiles+runs; the same conversion without `unsafe` is rejected; `Ptr<Int>→&String`, `&Int→&String`, and `&S→&mut S` rejected. Producer B: `test_resolve_opaque_type_qualified_arg_scope.py` (4) — `mem.Ptr<S>`→`Ptr<t.S>`, `other.Generic<S>` keeps outer in `other`/inner in caller, `other.Generic<third.S>` preserves `third.S`, qualified alias resolves its unqualified arg at the use site. `test_ctor_same_type_nested_unknown.py` (2) — a NESTED `Unknown` is not equivalent to a concrete type (no bypass). Producer C: `test_bare_name_local_nominal_precedence.py` (7) — local struct/generic/variant/interface/alias `Box` each win over the re-exported `core.Box`; explicit `core.Box<Int>` still resolves to the stdlib Box; bare `Box` with no local still resolves via the re-export fallback. Existing `test_variant_borrowed_match_construct_int_payload.py`, the typed-catch suite (31), the variant-ctor / ref-to-value suite (40), the alias/re-export/shadowing/box suites (32), the package suites (7), and the std.json entries-for-in e2e stay green.

## 2026-06-11 - 0.33.31: implicit `const_share` no longer desyncs callsite_ids from the per-callsite instantiation map (LANGUAGE_BUG, LB-8)
### LANGUAGE_BUG: cross-loop generic for-in mistargeting after a non-Copy ConstShare loop
- **Symptom:** a NON-Copy (move/const_share-path) for-in loop FOLLOWED by a generic by-value `Box<T>` for-in loop miscompiled — the generic loop's `iter()` callsite lowered to a MIR `Call` targeting `Box<T>::…::next__inst…` instead of `…::iter…`, so clang rejected the IR (`'%tN' defined with type '%Variant_main_Box…' but expected 'ptr'`: the iter position received the Box value where `next(self: &mut Box<T>)` wanted a ptr). Combination-dependent — `box`-only, `box`-then-`box`, and Copy-variant-then-`box` all pass; only a preceding non-Copy **ConstShare** loop breaks the following generic loop.
- **Root cause (first broken invariant):** `type_checker.check_function`'s `_alloc_callsite_id` seeds its high-water mark from `_max_callsite_id(body)`, which used a hand-rolled `__dict__` walker that did NOT descend into statement children of nested blocks (`HLet`/`HLoop` produced by the for-in desugaring). For a body whose calls all live inside for-in blocks it returned -1, so allocation started at **0**. The LB-5 secondary `&T→T` receiver coercion for the non-Copy loop synthesizes an implicit `const_share` HMethodCall and allocated it callsite_id **0** — colliding with an existing call. `_record_call_info`'s collision path reassigned it and the collision **cascaded +1 through every later call's callsite_id**, but the per-callsite instantiation map (`instantiations_by_callsite_id`, recorded during resolution keyed by the pre-cascade ids) was NOT shifted in lockstep — so the generic Box `iter()` node inherited the FOLLOWING `next()` monomorphization. Central resolution and HIR desugaring were both correct; the defect was purely in the checker's callsite-id bookkeeping (not MIR materialization / address-taken locals).
- **Fix, part 1 (root cause):** `_max_callsite_id` now uses the canonical `node_ids.iter_hir_walk` — the same complete traversal `assign_callsite_ids` uses to seed parse-time ids — so the high-water mark covers every call node and `_alloc_callsite_id` always allocates ABOVE existing ids. The implicit `const_share` (and every newly-allocated node) gets a fresh non-colliding id → no cascade → the mistargeting cannot occur. No for-in-specific materialization rule was added.
- **Fix, part 2 (collisions are invariant failures, not recoverable input):** the silent collision-recovery in the checker's callsite-id bookkeeping is removed. (a) `_record_call_info` collision identity is now the **owner node**, not CallInfo equality: first-claim and same-node re-record are the only accepted paths; a different node claiming an already-owned callsite_id raises an internal invariant failure naming the conflicting id and both node ids. The `existing == info` cross-node exception and the fresh-id reassignment branch are gone. (b) `check_function`'s start-of-check renumber switched from blanket `assign_callsite_ids` (overwrite-all, which silently renumbered duplicates away) to `node_ids.assign_missing_callsite_ids` (fill ONLY missing ids above the high-water mark; never repair existing) — so it trusts the unique dense ids produced upstream and a genuine duplicate surfaces rather than being masked. The fill is now called UNCONDITIONALLY (when a callable registry is present), replacing the prior hand-rolled "has any call" scanner — that scanner was the SAME incomplete `__dict__` walker that did not descend into nested-block statement children, so it could report "no calls" for a body whose calls all live in nested blocks, skip the fill, and let a synthesized generic call reach `record_instantiation` with `callsite_id=None` (losing the monomorphization request); `assign_missing_callsite_ids` walks with the canonical `iter_hir_walk` and is itself a no-op when nothing is missing. (c) the obsolete `prev_csid != csid` instantiation-migration on the HMethodCall record path is removed (dead once `_record_call_info` stops reassigning; it carried an owner-stealing hazard). Rationale: resolution may overwrite the owner's per-callsite side-table entries before a collision is detected, so recovery cannot determine which instantiation belongs to which node — fail at the first broken invariant. An unconditional migrate-on-reassignment band-aid was prototyped and rejected for that reason.
- **Telemetry:** the reassignment / cross-node paths were instrumented and swept (full 1293-case codegen-e2e check-only sweep + partial full-compile e2e/driver runs); neither fired in real compilation. A second check-only sweep over all 1293 sources after the fail-loud + fill-missing changes produced zero duplicate-callsite_id failures. Instrumentation removed.
- **Version:** `DRIFTC_VERSION` 0.33.30 → **0.33.31**; **ABI stays 16** (checker bookkeeping correction; no compiler/runtime boundary signature, layout, or calling-convention change).
- **Regressions:** e2e `for_in_nc_loop_then_generic_box_loop` (minimal failing combination — non-Copy loop then generic Box loop; full compile/run, exit 0, memcheck-clean); e2e `for_in_distinct_generic_insts_around_const_share_loop` (normal-path monomorphization separation — two DISTINCT generic instantiations `Box<Int>`/`Box<Bool>` around a const_share loop keep their own monomorphizations); driver `test_for_in_const_share_callsite_id_alignment.py` (two MIR-boundary structural pins); driver `test_node_ids_and_callinfo.py` (duplicate-callsite_id contract failures — distinct nodes sharing an id with different AND with identical CallInfo both raise, message naming id + both node ids; replaces the obsolete `*_are_reassigned_per_call_node` test — plus a focused `assign_missing_callsite_ids` test proving existing ids are untouched and missing NESTED-block ids are filled above the high-water mark). The for-in cases fail before the fix (LLVM type error / `rc != 0`).

## 2026-06-10 - 0.33.30: central `&T→T` receiver coercion + ConstShare-synthesis pipeline convergence (for-in by-value Iterable, final)
Completes the by-value `Iterable` for-in ownership matrix begun in 0.33.29.

### Central secondary `&T→T` receiver coercion (LB-5)
- `for x in v` where `v` is a **bound local** of a by-value-`Iterable` type was rejected (`E-NOT-ITERABLE`): central receiver matching had no `&T→T` path for a by-value `self`. Added a **secondary receiver-coercion** in the central method resolver (`call_resolver`), consulted only after exact/borrow candidates fail. The for-in desugar's **implicit** borrow carries a new `HBorrow.for_iter_implicit_borrow` provenance (set only by the default `for x in v` synthesis, never by a source-written `for x in &v`); the coercion **unwraps** that synthetic borrow to a fresh `HVar` read of the underlying projection-free local — Copy → the bare read (implicit-copy semantics), ConstShare-only → an implicit `const_share()` on that read — and is typed exactly once, the result becoming the candidate-local effective receiver type used through the continuation (infer / instantiate / autoborrow / args-match). Explicit `&v` stays borrow-mode; non-Copy bound locals still require `move`.
- **Ownership matrix** (e2e, memcheck): Copy bound local iterated **twice not consumed** (`for_in_byvalue_copy_local_reuse`→12), **ConstShare-only** local → independent owners, leak-clean (`for_in_byvalue_constshare_local`→12), plus the prior rvalue-temp move / borrow-only-array / `move v` / non-Copy-reject pins.

### LANGUAGE_BUG: ConstShare structural synthesis missing in the test pipeline (LB-7)
- Post-link ConstShare **structural synthesis** (derives the ConstShare impl for composition-rule user types + registers `const_share`) ran in the CLI but NOT in the test-helper pipeline (`compile_stubbed_funcs` → `compile_to_llvm_ir_for_tests`), so user types proved ConstShare in the CLI world but not the test world (the ConstShare for-in case passed `--entry` but failed the strict e2e runner). Extracted one **shared idempotent helper** `_run_post_link_const_share_synthesis` invoked by both paths pre-typecheck. Fixes uncovered while wiring: the synthesizer must receive the **local** callable registry the checker resolves against (not `semantic_world`'s), and the test-helper registry arrives **frozen** (unfreeze for the run). Idempotence is split — **semantic synthesis** run-once per `TypeTable`, **registry population** gated per-registry (`_const_share_hydrated`) so a prepared `TypeTable` reused with a fresh registry still hydrates.
- **Version:** `DRIFTC_VERSION` 0.33.29 → **0.33.30**; **ABI stays 16** (internal lowering + pipeline correction, no boundary change). Also: required-trait matching now compares full canonical `TraitKey` (package_id + module + name).

## 2026-06-10 - 0.33.29: fix for-in lowering for by-value variant Iterable (LANGUAGE_BUG); std.json Slice 2B — `JsonEntriesIter` for-in
### LANGUAGE_BUG: for-in over a by-value variant `Iterable` (codegen)
- **Symptom:** `for x in <rvalue producing a variant that implements iter.Iterable by value>` failed in clang — the `iter()` call passed the iterable's alloca **pointer** where the variant was expected **by value** (`'%__for_iterable_addr' defined with type 'ptr' but expected '%Variant_…'`). Structs/arrays survived (their by-value ABI *is* by-pointer); variants are LLVM values, so the `ptr` was a type error. Independent of std.json (minimal `pub variant V { … }` + `Iterable<V,_,V> for V` reproduces).
- **Root cause:** the for-in desugar always passed the iterable as a shared **borrow** `&Src`. Correct for borrow-iteration (`Iterable for &Src`, e.g. Array/String) and for struct/array temps (by-pointer ABI), but a type whose only `Iterable` is by value (`Iterable for Src`, e.g. a self-iterating variant) got the borrow's pointer where its by-value param expected the value.
- **Fix (two parts; resolution-driven, NOT syntactic).**
  1. **HIR-boundary marker.** The desugar marks the borrow of a freshly-bound, compiler-owned iterable temporary with `HBorrow.for_iter_owned_temp=True` (and makes that temp `var`). The `HBorrow` reconstructors in `normalize_hir` were rebuilding the node by hand and silently dropping the flag (`place_canonicalize.py`, `borrow_materialize.py`, `ast_to_hir._rename_expr`); switched them to `dataclasses.replace(expr, subject=…)` so flags (`allow_rvalue`, `for_iter_owned_temp`) survive to type-checking.
  2. **Trait-constrained selection + ownership coercion.** for-in resolves through the CENTRAL method resolver (`resolve_method_call`), extended with a hard `required_trait_key` so candidate selection admits only impls of the requested trait — never an inherent/unrelated same-named `iter()`/`next()` — while preserving the resolver's existing visibility, `require` proof, ambiguity, generic-substitution, and auto-deref handling. (An interim custom `trait_impl_index.get_candidates` scan was rejected in review and removed; selection stays inside the one central resolver.) Concrete for-in calls require a DIRECT MIR target, so a TRAIT result is retargeted to the resolved `MethodResolution.decl.fn_id` (signature preserved). Move-vs-borrow follows the selected impl: borrow-mode keeps the borrow; by-value `Src` from a marked owned **temp** is moved; the remaining `&T→T` for a bound local is the central secondary coercion (see 0.33.30). `required_trait_key` compares full canonical `TraitKey` (package_id + module + name).
- Internal lowering only — **ABI stays 16**; `DRIFTC_VERSION` 0.33.28 → **0.33.29**. (Earlier review rounds rejected an unconditional AST-stage move — it consumed user locals and regressed borrow-only rvalues like a non-Copy `Array<String>` temp — and a global `iter`-name signature scan that bypassed trait identity; both replaced by the above.)
- **Regressions:** stage1 `test_for_in_marker_and_selection.py` (the `for_iter_owned_temp` marker + `for_iter` origin survive `normalize_hir`, place → `__for_iterable`; inherent `iter()` ⇒ `E-NOT-ITERABLE`; non-Copy bound local w/o move ⇒ typecheck `E-NOT-ITERABLE`). e2e under memcheck: `for_in_iterable_ownership_matrix` (Copy variant rvalue; Copy local iterated TWICE = not consumed; **non-Copy variant rvalue moved + leak-clean String drop**; `for x in move v`; generic by-value `Iterable`), `for_in_byvalue_variant_iterable`, `for_in_rvalue_borrow_only_iterable` (non-Copy `Array<String>` rvalue still borrows), `std_json_entries_for_in`. Plus `test_for_in_byvalue_variant_iterable.py` (next()-only ⇒ `E-NOT-ITERABLE`). Existing for-in/iterable driver suite + full codegen e2e sweep green.
### LANGUAGE_BUG: for-in loop-variable NAME reuse — match-binder MIR identity (found during the above; fixed)
- **Symptom:** reusing a for-in loop-variable name across sibling loops in one function — `for x in a {…} for x in a {…}` — failed with `SSA: load before store for local 'x__b<id>'`. Reproduced on the borrow/array path, independent of the by-value work.
- **Root cause:** the match-binder MIR lowering (`hir_to_mir.py`) stored the binder under the **raw** `arm.binders` name, while binder **reads** use the binding-id canonical local (`_canonical_local`). A second loop reusing a name got a distinct binding-id → reads canonicalized to `x__b<id>` but the store still wrote raw `x` → SSA correctly rejected the read of an undefined local. (SSA was right; the producer-side `const_share_synth` binder-name localization had been a workaround for this same invariant class.)
- **Fix:** the binder loop now pairs `arm.binders` with `arm.binder_ids` and canonicalizes the destination once — `binder_local = self._canonical_local(binder_id, name)` — using it consistently for `ensure_local`, `_local_types`, `_register_drop_local`, and `StoreLocal` (legacy HIR without binder ids falls back to the raw name via `binding_id=None`). MIR lowering no longer depends on special binder names for correctness. Internal lowering only — ABI unchanged.
- **Regressions:** e2e `for_in_loop_var_reuse` (reused name across loops: Copy `&Int` + non-Copy `Array<String>` (leak-clean) payloads + early break/continue, under memcheck); stage2 structural `test_for_in_loop_var_reuse_mir.py` (captures pre-SSA MIR and asserts no `LoadLocal`/`MoveOut` reads a local without a matching `StoreLocal`/param — the binder store/read identity invariant).

### std.json Slice 2B: `JsonEntriesIter` is `Iterable` (`for entry in node.entries()`)
- Added `implement iter.Iterable<JsonEntriesIter, containers.HashMapItemRef<String, JsonNode>, JsonEntriesIter> for JsonEntriesIter` (self-iter: `iter(var self) -> self`). `entries()` keeps its signature (`-> JsonEntriesIter`), empty-iterator-for-non-objects behavior, borrowed keys/values (no cloning), unspecified order, and the existing `SinglePassIterator.next()`. NOT converted to a struct (the codegen defect was fixed at the root instead).
- **Consumption:** `for entry in node.entries()` works directly (no extra imports); manual `.next()` still requires `import std.iter as iter; use trait iter.SinglePassIterator;` — both documented on `entries()` and in `drift-stdlib-spec.md`.
- **Regressions** (memcheck): e2e `std_json_entries_for_in` (for-in over non-empty object with borrowed key/value access, empty/non-object → zero iterations, manual `next()` with the trait, leak-clean early loop-exit + iterator drop).

## 2026-06-10 - Slice 2A: document existing `JsonNode.entries()` (discoverability; docs + tests only)
- **No code-behavior / version / ABI change.** `JsonNode.entries(&self) nothrow -> JsonEntriesIter` already existed and works (borrowed `(key, value)` enumeration over `JsonEntriesIter` / `containers.HashMapItemRef<String, JsonNode>`); the app team missed it because it was undocumented. This slice closes the documentation gap and strengthens coverage — it does **not** change the signature (it stays `JsonEntriesIter`, not `Optional<…>`: a non-object and an empty object both yield no entries; there is no distinct "not an object" signal).
- **Documented**: an API doc comment on `entries()` (`stdlib/std/json/json.drift`) — borrowed keys/values (no cloning), **unspecified iteration order** (not canonical; use `encode_canonical`), empty-iterator-for-non-objects, and HashMap borrowing/invalidation rules; a `### Object entry enumeration` subsection in `doc/design/drift-stdlib-spec.md`; and the `entries()` signature/behavior in `doc/effective-drift.md` (was a bare mention).
- **Regressions** (`lang/tests/codegen/e2e/std_json_entries_iter_behavior`, under memcheck): non-empty object (complete enumeration), empty object (no entries), non-object/null (no entries), and a **leak-clean early iterator drop** (advance once, abandon, with the borrowed object still live).

## 2026-06-09 - 0.33.28: std.json Slice 2 — orthogonal parser policy, located decoder surface, canonical encoding; nothrow `HashMap.insert_if_absent`
### std.json: orthogonal parser policy (`parse_with_config` / `parse_strict`)
- **`JsonParseConfig`** (all `Copy`): `DuplicateKeyPolicy` (Reject/KeepFirst/KeepLast), `TopLevelPolicy` (AnyValue/ObjectOrArray/ObjectOnly), `JsonNumberPolicy` (independent `allow_fractions`/`allow_exponents`/`allow_negative_zero` — each selects a SUBSET of valid JSON), `JsonLimits` (`Optional<Int>` document-bytes/depth/string-bytes/number-bytes/array-items/object-fields).  Profiles `permissive()` / `strict()` / `signed_ir()` + `JsonParseConfigBuilder` (`build() -> Result`, validates negative limits → `"invalid-config"`).  All policies orthogonal; a config only RESTRICTS standard JSON or selects among duplicate-key interpretations.
- **`parse()` preserved EXACTLY**: re-pointed at the unified parser under a private legacy profile (keep-last dups, lenient numbers INCLUDING leading zeros via an internal `_ParseCtx.accept_leading_zeros` flag that is never a public toggle).  Leading zeros are invalid JSON and rejected by every public config.  Existing json regressions stay green (compatibility gate).
- **Numeric policy inspects the verbatim lexeme** (`JsonNode::Number(raw)`) before any conversion; precedence leading-zero → negative-zero → fraction → exponent.  Stable error tags: `duplicate-key` (offset = the second key's `"`, `key` = the key), `top-level-not-object` / `-object-or-array`, `number-{leading-zero,negative-zero,fraction,exponent}`, `limit-{document-bytes,depth,string-bytes,number-bytes,array-items,object-fields}`, `invalid-config`.
- **Duplicate-key probe strategy** (one probe/field): KeepLast → `insert()` (overwrite); Reject → `contains_key()` after the key, **before** parsing its value (immediate failure at the second key); KeepFirst → `insert_if_absent()`.  New `HashMapCore.insert_if_absent(var K, var V) -> Bool` (`stdlib/std/containers/array.drift`): inserts+`true` when absent, else drops the supplied key/value and returns `false`.  Probes presence first (one `find_slot`); the absent path reuses that slot unless growth rehashed (capacity changed), re-probing only then — so one probe for present and for absent-without-growth, two only on a rehash.

### std.json: located decoder surface (`parse_located` → `JsonDoc` → `LocatedCursor`)
- **`parse_located(text, cfg) -> Result<JsonDoc, JsonErrorData>`** retains a private `_SpanTree` sidecar (byte spans + per-object ordered key-occurrence list + value-span map mirroring the dup-key policy) plus the source text.  Built only in located mode — `_ParseCtx.locate` gates an optional span out-param threaded through the SAME helpers, so `parse`/`parse_with_config` skip all span work and are byte-identical.  Pinned: `parse_located(x,cfg).root` encodes identically to `parse_with_config(x,cfg)` for every public config (sidecar is value-neutral).
- **`LocatedCursor`** (all `nothrow`, all `Result`): `child`/`index`/`require_field`/`optional`/`forbid_unknown`/`discriminant`/typed getters (`as_string`/`as_int`/`as_uint`/`as_float`/`as_bool`/`as_object`/`as_array_len`)/`span`/`pointer`; `JsonDoc.at_pointer` is **absolute** (RFC-6901, `~0`/`~1` escaping, array-index grammar `0|[1-9][0-9]*`).  Object-contract error tags: `missing-field`, `unknown-field` (earliest occurrence in source order), `type-mismatch-{string,int,uint,float,bool,object,array}`, `invalid-pointer`, `invalid-array-index`, `index-out-of-range`.  `optional` is `Result<Optional<…>, …>` — `Ok(None)` only for an absent key, `Err(type-mismatch-object)` on a non-object.  (`require` is a reserved keyword → the helper is `require_field`.)

### std.json: RFC string parsing + post-review correctness fixes
- **RFC string rules for public configs**: `_parse_string` now decodes `\uXXXX` (incl. surrogate pairs → UTF-8) and **rejects** unescaped control bytes `U+0000–U+001F` (`unescaped-control`) under public configs. Legacy `parse()` stays bug-compatible (still rejects `\u` as `invalid-escape`, still tolerates unescaped controls) — gated by the renamed `_ParseCtx.legacy` flag (was `alz`). New helpers `_hex_val`/`_parse_hex4`/`_append_utf8`.
- **`max_object_fields` counts member OCCURRENCES** (not unique map size) — a flood of duplicate keys can no longer bypass the limit.
- **Limits enforced BEFORE consuming the oversized item**: array-item and object-field limits checked at the offending element/key start before parsing it; `max_string_bytes` and `max_number_bytes` enforced incrementally during the scan (no longer parse the whole oversized value first).
- **JSON Pointer escape validation**: a `~` not followed by `0`/`1` (incl. trailing `~`, `~2`) ⇒ `invalid-pointer` (`_jp_unescape` now returns `Optional<String>`).
- **`HashMap.insert_if_absent` no longer mutates on a present key**: presence is probed FIRST (safe pre-grow under load-factor < 1), so a duplicate triggers no rehash; `ensure_capacity` runs only on the absent path, which reuses the presence-probe slot unless capacity changed (rehash). Holding an iterator across the call is statically forbidden (`HashMapIter` borrows `&HashMapCore`), so iterator invalidation by this API is unreachable in safe code; the e2e pins post-call map integrity. (Allocation failure returns `false` + drops inputs — an OOM edge that cannot be forced from Drift, so it's by-construction, not unit-tested.)
- **`_int_to_string(Int::MIN)`** fixed — accumulates from the non-positive side, no overflow on negation (correct `index-out-of-range` diagnostic key).
- New error tag: `unescaped-control`. Coverage: e2e `std_json_rfc_strings_limits` + `hashmap_insert_if_absent` (both under memcheck).

### std.json: canonical encoding (`encode_canonical`)
- **`encode_canonical(node) -> Result<String, JsonErrorData>`** — a fresh **result-propagating** encoder (NOT the `_encode_node` object path that swallows failures to `"{}"`).  Fixed canonical form: UTF-8, no insignificant whitespace, object keys recursively UTF-8-byte-sorted (reuses `_string_lex_cmp` + the frozen `_encode_string` escape table for keys and values), signed-IR integer grammar on emit — rejects `-0`/leading-zeros/fractions/exponents with `canonical-number-{leading-zero,negative-zero,fraction,exponent,invalid}` and `canonical-invalid-node` for `Tombstone`/unsupported nodes (never a fallback).  Errors carry a JSON Pointer `path` + the offending raw in `key`; `offset = -1`.  Deterministic first-error in canonical emit order (array index order / sorted keys).  Hashing stays caller-side over the emitted bytes.

### Coverage + version
- **e2e (under `DRIFT_MEMCHECK=1`)**: `std_json_parse_policy`, `std_json_located`, `std_json_canonical`, `std_json_located_invariant` (JsonDoc/_SpanTree/LocatedCursor drop leak-clean).  **driver**: `test_std_json_parser_policy_api.py`.  Existing `std.json` + container regressions stay green.
- **Version**: `DRIFTC_VERSION` 0.33.27 → **0.33.28** (additive stdlib surface, patch).  **ABI stays 16** — pure-Drift stdlib, no boundary change.

## 2026-06-09 - 0.33.27: std.source + std.parse frontend toolkit (Slice 1); nothrow `Deque.get`
### std.source: UTF-8 scalar cursor + durable source locations
- **New module** `std.source` (`stdlib/std/source/source.drift`): `SourcePos` (`byte_offset`/`line`/`column`, `Copy`), `SourceSpan` (`source_id`/`start`/`end`, `Copy`), `SourceError` (`code` + zero-width `span`), and `SourceCursor`.  Byte offsets are authoritative for slicing/hashing; `line`/`column` are 1-based diagnostic coordinates and `column` counts **Unicode scalar values**, not bytes.  `source_id` is logical/configured (never an absolute path) and may be persisted into IR — same bytes + same `source_id` ⇒ byte-identical spans/slices/diagnostics across machines and reloads.
- **Eager validation** at construction: `source_cursor(&Array<Byte>, source_id)` runs one O(n) UTF-8 pass (RFC-3629 class boundaries; over-long + surrogate exclusions per leading byte) and on the first invalid byte returns `Err(SourceError{code = "invalid-utf8", ...})` with a zero-width span at that byte (line/column counted up to it; the offset is the bad byte, the line/column are the start of the offending sequence).  After success every offset the cursor yields sits on a scalar boundary, so `peek`/`advance` are infallible (`-1` only at EOF).  `source_cursor_from_string` skips re-validation (a Drift `String` is already valid UTF-8).
- **Cursor surface** (all `nothrow`): `position`, `at_end`, `peek`/`advance` (scalar value or `-1`), `mark`/`span_from`/`span_here`, `source_id`, `byte_length`, `slice(start,end)`, `slice_span(&SourceSpan)`.  Line/column tracked incrementally (O(1) at any point), vs `std.json`'s O(n)-per-error rescan.
- **CRLF (frozen, two-step)**: only `LF` advances the line; `CR` is an ordinary scalar; `CRLF` is two `advance` calls and the line advances exactly once, on the `\n`.  The cursor never hides a byte.
- **Slicing rejects**: out-of-range/inverted (`invalid-slice-range`), an offset that splits a multibyte scalar — continuation byte `0x80–0xBF` at `start`/`end` (`slice-not-char-boundary`), and a `slice_span` whose span `source_id` differs from the cursor's (`span-source-mismatch`).

### std.parse: pull-style token stream + structured diagnostic (additive)
- **New surface** on `std.parse`, purely additive — the scalar parsers (`parse_int`, …, `*_bytes`, `ParseError`) are unchanged and re-exported verbatim.  `trait TokenKind require Self is cmp.Equatable { fn describe(&Self) -> String }`; `Token<K> { kind, span: source.SourceSpan }`; `ParseDiagnostic { code, span, expected: Array<String>, found: Optional<String> }` (`found = None` is the structured EOF encoding; `code` set is stable, descriptor prose is not); `TokenStream<K> require K is TokenKind` owning a `containers.Deque<Token<K>>` (`Destructible`).
- **Stream methods** (all `nothrow`): `peek(n) -> Optional<&Token<K>>` (lookahead borrow of the n-th remaining token; no fixed bound), `current()`, `advance() -> Optional<Token<K>>` (by move), `at_end()`, and `expect(&K, name) -> Result<Token<K>, ParseDiagnostic>` — consumes-or-diagnoses (`unexpected-eof` / `unexpected-token`) **without consuming on mismatch**.  `expect` extracts the matched/`describe`/span values inside the immutable-borrow scope before the mutating `pop_front`, so the immutable lookahead borrow and the mutable consume never overlap.  No parser combinators / generator in this slice.
- **Token source**: this slice's `TokenStream` wraps a pre-lexed `Array<Token<K>>` (no pluggable lexer interface — the earlier draft's existential `Lexer<K>` field, and its SP-1 feasibility gate, were dropped from the design).

### std.containers: nothrow `Deque.get`
- **`Deque.get(i: Int) nothrow -> Optional<&T>`** added (mirrors `HashMap.get`): the bounds-checked, borrow-returning accessor for callers that must not throw.  `Deque.at` throws `IndexError`, and a `nothrow` fn cannot call a throwing fn (only `@intrinsic`s like `string_byte_at` are exempt), so the token stream's `nothrow peek` needs this.  Uses `mem.ptr_at_ref` directly under a guard.

### Coverage + version
- **e2e (under `DRIFT_MEMCHECK=1`)**: `std_source_cursor_basics` (ASCII walk, multibyte scalar-columns, two-step CRLF, EOF), `std_source_invalid_utf8` (rejection at exact byte offset), `std_source_slice` (bounds + mid-scalar + cross-source), `std_parse_token_stream` (lookahead/peek idempotency, advance, at_end, `expect` Ok + expected/found Err, leak-clean drop with a token still buffered).  **driver**: `test_std_source_parse_frontend_api.py` (compile-level pins of the public surface incl. an `expect` over a generic `TokenKind`; no program is run — runtime token destruction is the e2e's job) and `test_constshare_generic_field_frontend.py` (HIR-level pin that a ConstShare-eligible kind gets the implicit `const_share()` rewrite into `Token<K>`, while a non-ConstShare `Array<String>` kind requires `move` regardless of `std.core`).  Existing scalar `std_parse_*` cases and `std.json`/container regressions stay green.
- **Version**: `DRIFTC_VERSION` 0.33.26 → **0.33.27** (additive stdlib surface, patch).  **ABI stays 16** — pure-Drift stdlib, no compiler↔runtime boundary change.

## 2026-04-16 - 0.27.203: std.log `ContextResolver.resolve` returns `Optional<&LogContext>` (zero-copy hot path); `LogEnvelope` snapshots caller-thread IDs at emit time
### std.log: caller-thread-ID snapshot moved from drain time to emit time
- **Defect** (app-team-reported the morning of 0.27.203): `std.log` records were carrying the **drain worker**'s `vtid` / `ptid` instead of the **emitter**'s.  `LogEnvelope` stored only `payload_json`; `_emit_envelope_handle` called `thread.vt_id()` / `thread.posix_thread_id()` at drain time.  For async logging with queue-based drain, that means downstream tooling (which assumes `vtid` / `ptid` identify the event origin, like every other record field — ev, logger, level, attrs) gets whichever VT happened to dequeue the envelope, not who emitted it.  Unusable for request-flow correlation.
- **Fix**: `LogEnvelope` now carries `vtid: Int, ptid: Int` alongside `payload_json`.  `_emit_payload_throwing` snapshots `thread.vt_id()` / `thread.posix_thread_id()` synchronously BEFORE `_alloc_envelope` — the record's thread identity is fixed on the caller's stack, before enqueue.  `_emit_envelope_handle` reads `env.vtid` / `env.ptid` and formats those; it never probes the thread intrinsics itself.  The drain worker is an implementation detail of the queue/flush path; it is not metadata the logger should ever surface.
- **Regression coverage** (`lang/tests/driver/test_std_log_envelope_thread_id_snapshot.py`): structural snapshot-point pins — (1) `LogEnvelope` carries `vtid: Int` and `ptid: Int` fields; (2) `_emit_payload_throwing` calls `thread.vt_id()` and `thread.posix_thread_id()` before `_alloc_envelope`; (3) `_emit_envelope_handle` does NOT call either thread intrinsic and DOES read `env.vtid` / `env.ptid`; (4) `_alloc_envelope` accepts `vtid: Int, ptid: Int` parameters.  An end-to-end cross-thread test that forces a spawned VT to emit while a different VT drains is impractical without exposing the drain-lock; the structural test catches any regression that moves the snapshot point back into the drain path.

### std.log: `ContextResolver.resolve` returns `Optional<&LogContext>` (zero-copy hot path)
- **API shape change** (no `_dev` users on tip yet, no public release): `pub interface ContextResolver { fn resolve(self: &Self) nothrow -> Optional<&LogContext> }`.  Previously the return was owned `Optional<LogContext>`, which forced resolver impls to construct a fresh context per emit — fine when the resolver carries its own state, but fatal for the natural app pattern where request context lives in `rt.ScopedStack<log.LogContext>` inside a thread-local registry: `peek()` yields `&LogContext` and stdlib offered no `LogContext.clone` to convert borrow → owned.  App team filed `/tmp/stdlib-log-context-resolver-clone-gap.md` the morning of the 0.27.202 integration.
- **Why borrowed, not `clone`**: the logger reads the borrowed context synchronously during record formatting, serializes its attrs into the owned `payload_json` String stored in the queued `LogEnvelope`, and does not retain the borrow after enqueue or after the emit call returns — the safety boundary is "before enqueue / before emit returns", not "before `resolve` returns".  No ownership transfer is needed.  `LogContext.clone` may ship later as independent API for apps that genuinely need owned context, but it must not be on the resolver hot path.  Verified empirically: Drift's borrow checker accepts resolver impls that return a borrow rooted in thread-local / registry state (not in `&self`) — the thread_registry + get<T> + peek() chain typechecks cleanly as `Optional<&LogContext>`.
- **Implementation**: interface return type flipped; `NoContextResolver.resolve` returns `Optional<&LogContext>::None()`; `_resolve_ctx_from_state` returns borrowed; bare-emit paths (`_emit<V>`, `_emit_no_attrs`) now feed the borrowed ctx into the same `_emit_ctx_*_throwing*` machinery the explicit-`&LogContext` overloads already used — no new emit helpers, and the owned-ctx `core.drop_value<type LogContext>(move ctx)` calls are deleted (no longer owned).
- **Fixture updated** (`lang/tests/codegen/e2e/std_log_resolver_active/main.drift`): `StdResolver` now pre-builds its `LogContext` at construction time (`std_resolver(user_tag, req_tag)`) and hands out `&self.ctx` on every `resolve()` — the simplest borrowed-storage shape.  `expected.json` unchanged (identical JSON output).
- **App-shape regression** (`lang/tests/driver/test_std_log_resolver_scoped_stack.py`): end-to-end pin for the exact app pattern — `RequestContextState { ctx: rt.ScopedStack<log.LogContext> }` installed in `rt.thread_registry()`; `AppLogResolver.resolve` consults `rt.thread_registry() → rt.get<RequestContextState> → st.ctx.peek()`; `logger.info("event")` picks up the pushed context, `logger.info("event", &log.log_context())` suppresses the resolver per-call, and `logger.info` after the `ScopeGuard` drops carries no ambient ctx.  Compiles + runs + asserts the JSON lines carry `req_id` / `user` only where expected.
- **Compiler-version policy**: bumps to 0.27.203 (behavior-changing stdlib signature).  ABI stays at 9 — no boundary shape change; the change is source-level in the interface method return.  `ContextResolver` impls written against 0.27.202 need one-line updates (return type + `Optional<&LogContext>::Some(&…)` instead of `Some(move …)`).

## 2026-04-16 - 0.27.202: Borrowed-interface dispatch (`(&iface).method()`, `arc.get().method()`); std.log resolver redesigned as `Arc<ContextResolver>` interface
- **LANGUAGE_BUG** (surfaced while sketching the std.log resolver as `Arc<ContextResolver>`): calling an interface method through a borrow — `(&iface).method()`, `arc.get().method()` — was rejected at `lang/driftc/checker/call_resolver.py:1597` with "interface method call requires a value receiver (remove '&')".  The wall forced the std.log v0.27.201 resolver to be a function pointer (no closure environment, no captures) and blocked the natural "shared trait-object service" pattern — `Arc<Interface>` couldn't dispatch without paying retain/release per call.
- **Regression-first** (`lang/tests/driver/test_borrowed_interface_dispatch.py`): three driver tests — two end-to-end (compile + run binary, assert exit code) covering `(&iface).method()` and the `Arc.get().method()` shape, plus an IR-level assertion that `arc.get().method()` contains no `atomic_fetch_add` / `atomic_fetch_sub` / `arc_clone` on the dispatch path.  The two runtime tests fail on `main` before the fix with the exact rejection diagnostic.
- **Three-layer fix**:
  1. **Checker** (`call_resolver.py:1593-1598`): drop the unconditional REF<INTERFACE> rejection.  The existing `recv_nominal` unwrap (a few lines below) already routes the receiver to the interface-dispatch branch at line 1628 — no other change needed.
  2. **HIR→MIR** (`stage2/hir_to_mir.py:_lower_method_call_with_info`): when `info.target.kind is INDIRECT` and the receiver type is REF<INTERFACE>, route to `_lower_iface_call` (which emits CallIface) instead of `_lower_indirect_call` (which would emit CallIndirect — function-value call — segfaulting at runtime when invoked on a fat pointer).
  3. **`_lower_iface_call`** (`stage2/hir_to_mir.py:7297`): unwrap REF<INTERFACE> to INTERFACE before extracting the interface-instance / vtable-slot.  The lowered `iface_val` is a pointer to the fat pointer; the LLVM `_lower_call_iface` lowering at `llvm_codegen.py:6183` already detects pointer arguments and loads through them — no codegen change needed.
- **No new MIR shape, no new vtable layout, no ABI change**: borrowed-interface dispatch reuses the existing `CallIface` instruction and the existing `_lower_call_iface` LLVM lowering.  All the work was making the checker / HIR→MIR path pick the right instruction kind for receivers it had previously refused.
- **Hot-path verified** (resolver-enabled emit): IR for `_resolve_ctx_from_state` is `GEP &resolver → Arc.get → load fat pointer → vtable[idx] → indirect call` — no `atomic_fetch_add` / `atomic_fetch_sub`, no `arc_clone`, no allocation.  `Arc.get` itself is a non-atomic pointer chase + bounds check.  Builder/build/sub-logger paths still pay one Arc.clone (atomic_fetch_add) at construction time only — never per emit.

### std.log: ContextResolver becomes a trait-object interface
- **`pub interface ContextResolver { fn resolve(self: &Self) nothrow -> Optional<LogContext> }`**: replaces the v0.27.201 `pub struct ContextResolver { f: Fn() nothrow -> Optional<LogContext> }` function-pointer wrapper.  Implementations may carry per-instance state (counters, request-id maps, downstream services) that the prior fn-pointer API forbade.
- **Storage shape**: `LoggerConfig.resolver: conc.Arc<ContextResolver>` (always present, no `Optional`).  Default is `NoContextResolver { resolve(&self) -> None }`, swapped in by `LoggerConfigBuilder.context_resolver(your_impl)`.  This is an intentional API shape, not a workaround: it gives the dispatch path a single uniform code shape (always `arc.get().resolve()`), avoids an extra Optional discriminant on the hot path, and makes the "no resolver configured" case observable as a concrete impl rather than a missing-value sentinel.
- **Builder API**: `pub fn context_resolver(self: &mut LoggerConfigBuilder, var r: ContextResolver) nothrow -> &mut LoggerConfigBuilder` takes any value implementing `ContextResolver` and wraps it in `conc.arc(...)`.  Caller writes:
  ```drift
  pub struct AppResolver { req_state: ReqState }
  implement log.ContextResolver for AppResolver { ... }
  b.context_resolver(AppResolver(req_state = ...));
  ```
- **`LoggerConfig::Copy` removed + checker hardened uniformly**: the stale Copy impl was the actual root cause of the UAF (not a field-projection codegen bug).  Bit-copying a struct with a non-Copy Arc field creates an unretained duplicate whose destructor double-frees.  **Fix**: (1) `_is_structurally_copy` in `type_checker.py:validate_trait_impls` now requires struct fields to be types whose own Copy impl is declared in the same compilation — so `Arc<Interface>` (no Copy impl, since Arc is Destructible) cannot satisfy a containing struct's Copy requirement; (2) the prior blanket `std.* / lang.*` stdlib exemption is **removed** — Copy validation now applies uniformly; (3) a narrow allowlist of compiler-known Copy types (`String`, `DiagnosticValue`) is permitted because their refcount-backed bit-copyability is not expressible by the structural prover; (4) generic Copy impls (e.g. `implement<T> Copy for Optional<T> require T is Copy`) still go through the structural prover — TYPEVARs covered by a conjunctive `require T is core.Copy` clause are treated as Copy, and a stored-field TYPEVAR not so covered rejects the impl; phantom generic structs (no stored T) pass with no require clause, and wrappers that don't propagate Copy-dep (RawPtr<T>, &T, fn ptrs) also pass without a require clause; (5) the covered-tparams collector resolves the clause's trait via `trait_key_from_expr` and accepts only canonical `std.core.Copy` — a shadowing local `trait Copy` in the same module cannot silently satisfy a `require T is Copy` clause on a `core.Copy` impl; (6) regression `lang/tests/driver/test_copy_impl_noncopy_field_rejected.py` pins seven shapes — the LoggerConfig Arc<Interface> case, the negative `implement<T> Copy for Box<T>` with stored T, the positive `implement<T> Copy for Pair<T> require T is core.Copy`, the positive wrapper-composition `implement<T> Copy for Outer<T> require T is core.Copy` where `Outer` holds a `Pair<T>` field (drills through Pair's substituted field_types and finds the covered TYPEVAR), the matching negative without require, the negative shadowing-local-`trait Copy`, and the positive phantom `implement<T> Copy for Handle<T>` (no stored T).
- **std.log uses the natural source shape**: `cfg.resolver.clone()`, `st.resolver.clone()`, `st.resolver.get().resolve()`, `self.resolver.clone()` — no `&self.field` workaround, no inline workaround comments.  The natural form works correctly now that LoggerConfig is move-only.
- **New regression coverage**:
  - `lang/tests/codegen/e2e/std_log_resolver_active/`: now uses a stateful resolver struct (`StdResolver { user_tag, req_tag }` implementing `ContextResolver`), exercising the bookkeeper-style "stash app state in the resolver instance" pattern that the fn-pointer API made impossible.
  - `lang/tests/driver/test_borrowed_interface_dispatch.py`: pins both the language fix and the std.log usage shape.
- **Always-present resolver design**: storage is `Arc<ContextResolver>` (no Optional) with a `NoContextResolver` default that returns `None`.  A logger with no custom resolver pays one indirect call per bare emit (vtable dispatch into `NoContextResolver.resolve` → returns None immediately) — negligible vs. JSON serialise + sink write.
- **Compiler-version policy**: bumps to 0.27.202 (behavior-changing fix in checker + MIR).  ABI stays at 9 — no boundary shape change; the existing `CallIface` MIR instruction and `_lower_call_iface` LLVM lowering are reused unchanged.  `LoggerConfig` is no longer `Copy` (breaking source-level change for any code that bound `cfg = b.build()` and reused it; existing fixtures all pass cfg inline once).

## 2026-04-16 - 0.27.201: std.log context resolver + collapse `_ctx` method family; FnResult LLVM-name sanitization
- **std.log API change** (breaking — no `_dev` users on this tip yet): `Logger.info_ctx` / `info_ctx_attrs` and the `debug` / `error` siblings are removed.  The context-bearing forms collapse into overloaded `Logger.info(ev, &ctx)` / `Logger.info(ev, &ctx, attrs)` (and same for `debug` / `error`).  The `(ev, &ctx, attrs)` form has both a typed overload (`HashMap<String, V> require V is Debuggable`) and a `DiagnosticValue`-typed overload — typed is the ergonomic clean path (`{"size": 42}`); DV is the fallback for mixed-type overrides (`{"k": DiagnosticValue::String(...), ...}`).  Three e2e fixtures updated (`std_log_context_scoped`, `std_log_context_nested_scopes`, `return_ref_registry_logger_helper_rejected`).  Macro family (`macro_info`, `macro_debug`, `macro_error`) updated in lockstep with both typed and DV ctx+attrs overloads.
- **Ambient context resolver**: new `LoggerConfigBuilder.context_resolver(f: Fn() nothrow -> Optional<LogContext>)` installs a process-supplied function pointer consulted by bare `Logger.info(ev)` / `info(ev, attrs)` (and `debug` / `error`) at emit time.  When the resolver returns `Some(ctx)`, its attributes merge into the record (caller-supplied attrs win on key collision).  Calls passing an explicit `&LogContext` bypass the resolver — that's the precedence rule.  Per-call opt-out is `logger.info(ev, &log.log_context())` (explicit empty context).
- **Why function pointers, not closures**: storing a `core.Callback0<...>` in the runtime state and calling it at emit time runs into a hard compiler restriction — `interface method call requires a value receiver (remove '&')` at `lang/driftc/checker/call_resolver.py:1597`.  Owning the Callback0 in `LoggerRuntimeState` and dispatching `cb.call()` would consume it on first use, breaking the per-emit contract.  Function pointers sidestep the dispatch limitation entirely.  Cost: no closure environment — apps that need per-Logger captures encode them into process- or thread-local state the resolver reads (this is the bookkeeper VT-context pattern verbatim).
- **`ContextResolver` is a struct wrapper, not a type alias**: a `pub type ContextResolver = Fn() nothrow -> Optional<LogContext>;` alias parsed cleanly but tripped a non-deterministic TypeId-interning issue across multi-pass parse (`test_emit_package_is_deterministic_with_changed_package_filenames`: LoggerConfig field 7 ID flipped between passes).  Wrapping in `pub struct ContextResolver { pub f: Fn() nothrow -> Optional<LogContext> }` interns deterministically (struct path) and gives us a name usable in `Optional<...>::None()` (the `Fn(...)` token isn't accepted in qualified-construction position).  The struct is `core.Copy` so `LoggerConfig::Copy` is preserved.
- **Resolver dispatch in emit path**: `_resolve_ctx_from_state` matches `st.resolver`, binds the wrapper, and rebinds the function pointer to a local before calling — `cr.f()` parses as a method call on the struct, so `val fp = cr.f; return fp();` is required.  `_emit_no_attrs` and `_emit<V>` consult the resolver and route through `_emit_ctx_throwing` / `_emit_ctx_attrs_throwing_typed<V>` when it returns `Some`.  Explicit-ctx Logger overloads route through `_emit_explicit_ctx` / `_emit_explicit_ctx_attrs` which never consult the resolver.
- **New typed-merge helpers** (`_emit_ctx_attrs_throwing_typed<V>`, `_payload_json_ctx_attrs_typed<V>`, `_ctx_attrs_json_merged_typed<V>`): mirror the existing DV-only family for the resolver+typed-attrs case.  Caller-supplied attrs win on key collision (matches the explicit `info(ev, ctx, attrs)` semantics).
- **Compiler bug exposed and fixed** (`lang/codegen/llvm/llvm_codegen.py:_declare_fnresult_named_type`): generic ok-payload type keys (e.g. `lang.core::lang.core.Optional<std::std.log.LogContext>`) were emitted verbatim into LLVM type names, producing `%FnResult_lang.core::...Optional<std::std.log.LogContext>_Error` which clang's IR parser rejects on the `<`.  Sanitizer now mangles non-alnum chars to `_` and appends a 16-hex hash suffix when sanitization is non-identity (collision-safe).  Hit by the new `Fn() nothrow -> Optional<LogContext>` indirect-call thunk in `_ensure_nothrow_wrap_thunk` (the standard `llvm_type_for_typeid` FnResult path was already pre-sanitizing; only the nothrow-wrapper-thunk path was raw).
- **Diagnostic quality regression** in the no-matching-overload path (`std_log_non_debuggable_attrs_rejected`): the post-collapse Logger overload set has two 2-arg `info(...)` candidates (typed-attrs `info<V>(ev, attrs)` and explicit-ctx `info(ev, &ctx)`).  When a caller passes a `HashMap<String, NonDebuggable>` as the second arg, the explicit-ctx candidate fails type-shape unification and the typed-attrs candidate fails the `V is Debuggable` constraint — but the resolver collapses both failures into a single compact `no matching method 'info' for receiver Ref<...Logger>` diagnostic instead of surfacing the constraint failure from the typed-attrs candidate (the only one whose param shape matched).  The test's expected.json is updated to accept the compact form; intent (non-Debuggable values are rejected) is preserved.  Follow-up: improve overload resolution to prefer the constraint-failure diagnostic when exactly one candidate's parameter types unify.
- **Compiler-focused regression** (`lang/tests/driver/test_fnresult_generic_ok_payload_llvm_name.py`): minimal stdlib-coupling-free repro — a struct field of type `Fn() nothrow -> Optional<Int>` initialized from a nothrow `produce()` function.  Storing the address triggers `_ensure_nothrow_wrap_thunk` with a `type_key_string("Optional<Int>")` ok-key.  Two tests: one regex-checks every `%FnResult_..._Error` declaration is a valid LLVM identifier; one belt-and-suspenders scan rejects any FnResult token containing `<`, `>`, `:`, `.`.  Both fail without the sanitizer fix and pass with it.  Includes `assert "@__nothrow_wrap_m_produce" in ir` so future code shape changes that move this code path elsewhere don't silently un-cover the regression.
- **New e2e fixtures**:
  - `std_log_resolver_disabled` — no resolver baseline.  Bare `info(ev)` produces empty attrs; bare `info(ev, attrs)` produces only those attrs.
  - `std_log_resolver_active` — resolver set; covers the full matrix the user listed: bare info → ambient ctx; bare info+attrs → ambient ctx + attrs (caller wins on key collision); explicit ctx → resolver suppressed; explicit ctx + DV-attrs → merge with attrs winning; explicit empty ctx → per-call opt-out.  Same patterns at `info`, `debug`, and `error`.
- **Versioning**.
  - `DRIFTC_VERSION` bumps from 0.27.200 to 0.27.201.
  - No ABI change: `DRIFT_RT_ABI_VERSION` stays at 9 — the resolver lives entirely in `LoggerConfig` and `LoggerRuntimeState`, both stdlib types behind `sync.Handle<Byte>`; runtime-helper signatures and boundary contracts are unchanged.

## 2026-04-16 - 0.27.199: Auto-try unification — compiler-owned `or_throw()`, `Try` trait / `into_try()` removed
- **LANGUAGE_BUG** (reported by web-rest consumer on 0.27.198): in a `throws` function, calls to package functions returning `Result<T, E>` where `E: core.Throw` failed overload resolution with "no matching overload" — the explicit `.or_throw()` form worked but the implicit `throws` auto-try did not.  Root cause: the old auto-try synthesized `Try::into_try()` (a trait method) and gated on `_try_trait_in_scope()`.  Without `use trait core.Try` in the consumer, auto-try never fired, so the Result<T, E> binding was never unwrapped and every downstream call on it failed.
- **Design cut — auto-try is now compiler-owned**:
  - Inside a `throws` function or `try {}` block, `Result<T, E>` expressions eagerly auto-unwrap to `T` via compiler-synthesized `.or_throw()`.  No trait import required.
  - Opt-out: an explicit `Result<T, E>` type annotation on a val binding preserves the Result so the user can call `.or_throw()` / pattern match explicitly.
  - Explicit `.or_throw()` remains the supported user-facing spelling.  It carries the `require E is Throw` constraint on its signature.
- **`Try` trait and `into_try()` method are removed from the language surface**:
  - `stdlib/std/core/copy.drift`: `pub trait Try<T>` deleted, `implement<OkT, ErrT> Try<OkT> for Result<OkT, ErrT> require ErrT is Throw` deleted.  `or_throw` body is self-contained (inlines the match that `into_try` used to do).  `Try` removed from export block and from `use trait` declarations.
  - Consumer code: every `use trait core.Try;` stripped from active stdlib-importing tests, examples, and e2e fixtures — the trait is not resolvable and the import is a no-op for auto-try anyway.
  - Negative regression `test_into_try_rejected` pins that explicit `.into_try()` is rejected as a method call.
- **Compiler changes** (`lang/driftc/type_checker.py`):
  - `_try_trait_in_scope()` helper removed entirely (was only called by the auto-try gate).
  - `_should_auto_try(expr_ty, expected_ty)` simplified: fires whenever `_auto_try_context()` and `expr_ty` is a `Result` variant, unless `expected_ty` is also a Result (the opt-out).  No trait scope consulted.
  - `_wrap_auto_try(expr)` (renamed from `_wrap_into_try`) synthesizes `or_throw()` unconditionally.
  - `HExprStmt` handler in auto-try context no longer emits the "Result value discarded in try-block (use 'use trait core.Try' or assign the value)" diagnostic; the expression auto-unwraps silently.
  - `_is_core_result_variant(ty)` hardened: falls back to TypeDef-name check for `VARIANT` and `FORWARD_NOMINAL` kinds, so unqualified `Result<...>` annotations in package-consumer contexts (where the annotation resolves to a FORWARD_NOMINAL pending link) still participate in the opt-out check.
- **`or_throw` API surface**: `pub fn or_throw(self: Result<T, E>) -> T require E is Throw { … }` — the `E is Throw` constraint is now on the method signature, not hidden on an internal `Try` impl.  Overload visibility and diagnostics surface the constraint directly.
- **Regression fixtures**:
  - `lang/tests/codegen/e2e/pkgb_throws_auto_try_result/` — NEW package-boundary e2e: producer exports `Result<Widget, AcmeError>` (AcmeError implements Throw), consumer in a `throws` function with zero trait import uses inferred bindings, `&mut` pass-through, and `move` consumption.  Exercises the original web-rest shape end-to-end.
...
- **Versioning**.
  ...
  - No ABI change: `DRIFT_RT_ABI_VERSION` stays at 9 — auto-try was always a source-level transform; the boundary shape of `or_throw` (signature, calling convention) is unchanged.
...
- **Review-iteration addendum**.
  ...
  - **Medium (caught by review + regression)**: the exception-ctor lowering wrongly treated `HDVInit` payloads as rvalue. `HDVInit` lowers to `M.ConstructDV`, whose caller-owned temp is already released at ConstructError / ErrorAddAttrDV sites via `_construct_dv_temps` / `_release_construct_dv_temp` (the 0.27.187/188 mechanism). Adding a MIR `DropValue` on top double-freed the payload — flaky SIGABRT in `exception_string_attr_concat_double_catch_no_corruption` when two sequential catches aliased the same freed String. Fix narrowed the double-release in TWO places, NOT symmetrically: (a) the DV-method-receiver allow-list `_DV_RVALUE_RECV_HEXPRS` keeps `HCall` / `HMethodCall` / `HIndex` / `HDVInit` (HDVInit stays for receivers like `DV::Object(...).entries()` because DVEntries / DVLen / DV-accessor ops never fire the `_construct_dv_temps` release, so a MIR-level DropValue is still required); (b) the exception-ctor path (`_construct_error_from_exception_init`) explicitly sets `dv_is_rvalue = False` for HDVInit (and the literal-promotion branch) because those values DO reach ConstructError / ErrorAddAttrDV, where `_construct_dv_temps` handles the release and an extra DropValue double-frees. Projected places (`holder.dv.entries()`) also excluded from the receiver allow-list — `extractvalue` is a shallow alias of the owning struct's storage. Added regression `dv_projected_place_entries_no_double_free` that SIGSEGVs pre-fix even in plain mode.
- **Final verification**. Full `lang/tests/codegen/e2e` sweep: 1134 passed, 0 failed. Full memcheck slice of the DV/array ownership regressions (9 fixtures): all pass.


- **Compiler fix (LANGUAGE_BUG×2): owned temp leaks on consumption paths exposed by 0.27.190's typed `.or_throw()`**.
  - **Leak A** — `__exc_attrs_get_dv` returns an independently-owned `DiagnosticValue` clone; downstream DV intrinsic methods (`as_int`, `as_bool`, `as_float`, `as_string`, `as_object`, `get`, `len`, `entries`) read the receiver via pointer in the runtime and do **not** consume it. When the receiver was an rvalue (e.g. `e.attrs["fields"].entries()`), the lowering produced an owned DV temp with no other owner — its refcounted payload (Object's `Array<DiagnosticEntry>`, Array's items, String buffers) leaked. Bound receivers (`val dv = …; dv.entries()`) were already safe via local scope-drop. Fix: `_dv_method_recv_is_rvalue` helper detects rvalue receivers (everything except `HVar` and projection-free `HPlaceExpr`); a `DropValue` is emitted on the rvalue DV after each DV method op.
  - Same Leak A pattern on the throw side: exception-ctor lowering passes each field DV into `ConstructError(payload=…)` / `ErrorAddAttrDV(value=…)`, both of which clone the DV at runtime (`drift_error_new_with_payload` / `drift_error_add_attr_dv` both call `drift_dv_clone`). Source rvalue DV temps were never released. Fix: per-field `DropValue` after `ConstructError`/`ErrorAddAttrDV` ONLY when the source is NOT a ConstructDV-backed value (i.e. only for HCall / HMethodCall / HIndex returning DV).  HDVInit payloads and literal-promoted DVs (HLiteralInt/Bool/String) both lower to `M.ConstructDV`, whose caller-owned temp is already released at Construct*/ErrorAddAttr* sites via `_construct_dv_temps` / `_release_construct_dv_temp` (the 0.27.187/188 mechanism) — so `dv_is_rvalue` stays False for those branches to avoid double-release.
  - **Leak B** — `_ensure_array_elem_copy` always emitted `CopyValue` for Copy non-bitcopy element types (the 0.27.39-dev MIR-validator contract for array stores) but never released the source temp. `CopyValue` deep-clones inner refcounted storage (String retain, DV clone) into the array's element copy, leaving the source's inner storage with the original refcount and no owner. `Array<DiagnosticEntry>` with heap-allocated keys was the observable case (string literals carry `DRIFT_STRING_FLAG_STATIC` and their release is a no-op, masking the bug in existing tests). Fix: emit `DropValue(value=val, ty=elem_ty)` immediately after the `CopyValue` in `_ensure_array_elem_copy` so the source temp's owned substructure is released. The cloned copy returned to the caller remains the lone owner of the array element.
- **Regression coverage**.
  - `lang/tests/codegen/e2e/exception_dv_object_rvalue_entries_no_leak/`: catches an exception carrying a non-empty `DV::Object` and exercises both rvalue (`e.attrs["fields"].entries()`) and bound (`val dv = …; dv.entries()`) receiver forms with a heap-allocated key. Fails under `DRIFT_MEMCHECK=1` pre-fix; passes post-fix.
  - `lang/tests/codegen/e2e/diagnostic_entry_array_struct_drop_no_leak/`: a struct owning `Array<DiagnosticEntry>` with heap-allocated keys, dropped both as a bare local and inside `Result::Err`. Same pre-/post-fix profile.
  - Re-ran adjacent DV regressions (`dv_exception_field_round_trip`, `dv_object_entries_len`, `diagnostic_entry_array_take`, `diagnostic_entry_array_field_read`, `exception_dv_attr_no_leak`) under `DRIFT_MEMCHECK=1`; all pass.
- **Versioning**.
  - `DRIFTC_VERSION` bumps from 0.27.190 to 0.27.191.
  - No ABI change: `DRIFT_RT_ABI_VERSION` stays at 9 because this is a compiler-side lowering change only, not a compiler/runtime boundary shape change.
