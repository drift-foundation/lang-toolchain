...
- **Fix:** added an `i32` branch gated to the six comparison ops (`EQ/NE/LT/LE/GT/GE`) → lowers to `icmp` producing `Bool`. `value_types` collapses `Int32`/`Uint32` to `"i32"`, so signedness isn't recoverable at this point: EQ/NE are signedness-agnostic (always correct), ordering uses signed semantics. Narrow-int **arithmetic** stays out of scope — the `int_ty is None` raise still rejects `i32` add/sub/etc. (per the reporter's request to keep that scope but not crash comparisons).
- **Version:** `DRIFTC_VERSION` 0.33.36 → **0.33.37**; **ABI stays 17** (stage-1 + codegen fixes; no runtime-boundary change).
- **Regressions:** driver `test_borrow_in_cast_no_double_free.py` (3) — cast-of-borrow-bearing-call runs clean, the reporter's nested-cast chain runs clean, and the val-bound control still works; `test_narrow_int_comparison.py` (3) — `Uint32 ==`, `Int32 ==`, and `Int32` ordering produce correct `Bool`. Both crash/abort before their fixes.
