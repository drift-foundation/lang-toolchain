...
- **Phase 1 — certified app artifacts:**
  ...
  - **`drift verify-app <app-dir>`** — read-only consumer CLI (mirrors `drift trust verify-package` trust flags / JSON / exit codes). **Never executes the binary**: app execution stays the orchestrator/service-manager's job after it independently verifies. There is no `drift run` / verify-then-exec command (explicitly out of scope).
