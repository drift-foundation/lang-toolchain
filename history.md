...
- Path-based whole-file helpers (opened-handle/streaming reads stay in `std.io`): `read_to_bytes(path, max_bytes, timeout) nothrow -> io.Result<Array<Byte>>` and `read_to_string(path, max_bytes, timeout) nothrow -> io.Result<String>`. `max_bytes` is mandatory and in bytes: `<0` → `IoError(kind="invalid-argument")` (no open); `==0` accepts only an empty file; an over-cap file — **including one that grows past the cap mid-read** — fails with the distinct `IoError(kind="too-large")` rather than truncating (the cap is enforced per chunk; no stat fast-path, which would need a new runtime boundary). `read_to_string` validates UTF-8 (`std.text.utf8_from_bytes`) and returns an OWNED String; invalid UTF-8 → `IoError(kind="invalid-utf8")`. Error precedence: read / decode / too-large win over a close error; a close-only failure returns the close error. Timeout/cancel/saturation surface via the underlying `io` read, consistent with `read_dir`; no implicit unbounded wait. Implemented purely over existing public `std.io`/`std.text` APIs → **no new runtime symbols**. Added `io.Result<T>` alias and `IO_ERROR_KIND_INVALID_ARGUMENT` / `IO_ERROR_KIND_TOO_LARGE` to `std.io`. Tests: `lang/tests/driver/test_std_fs_read_to.py` (contract + ownership-move valgrind).
