...
> * **ABI 12** — Slice 1 / Slice 2 (current).  Compiler emits calls to `drift_dv_kind`, `drift_dv_index`, `drift_error_get_params_json`, `drift_error_set_params_json`, `drift_error_get_context_json`, and `drift_error_append_context_frame`.  `<error>.params.encode_compact()` returns the canonical params JSON document; `<error>.context.encode_compact()` returns the canonical context JSON array.  All six helpers were Phase 1 substrate; Slice 2 adds compiler-emitted calls to the two context helpers, no new runtime surface or signature changes.  Runtime archive must export the helper surface; the link-time ABI stamp guards against version skew.
