"""Protocol-v5 implementation for the Baton filesystem handoff tool."""
...
class Mailbox:
	...
	def _load_roles(self) -> dict[str, dict[str, str]]:
		...
		if set(data) != {"protocol_version", "roles"} or data["protocol_version"] != PROTOCOL_VERSION or not isinstance(data["roles"], dict):
			raise MailboxError(f"Baton role configuration must declare protocol_version {PROTOCOL_VERSION} and roles")
	...
	def _validate_v5(self, path: Path) -> tuple[dict[str, Any], Path | None]:
		message_stat = _regular_nonsymlink(path, label="mailbox message")
		if message_stat.st_size > ENVELOPE_MAX_BYTES:
			raise MailboxError(f"message envelope exceeds {ENVELOPE_MAX_BYTES} bytes: {path.name}")
		...
		if match is None:
			raise MailboxError(f"not a protocol-v5 mailbox name: {path.name!r}")
		try:
		...
		required = {"protocol_version", "message_type", "message_id", "from_role", "to_role", "created_at", "retention", "kind", "thread_id", "outcome", "responds_to", "author_actor", "author_seed"}
		if message_type == "notice":
			required.add("expires_at")
		retention = envelope.get("retention")
		if retention == RETENTION_DURABLE:
			required.add("target")
		elif retention == RETENTION_TRANSIENT:
			required.add("body")
		else:
			raise MailboxError(f"message contains an invalid retention policy: {path.name}")
		if set(envelope) != required:
			raise MailboxError(f"message envelope fields do not match the v5 {message_type}/{retention} contract: {path.name}")
		if envelope["protocol_version"] != PROTOCOL_VERSION or envelope["message_type"] != message_type:
		...
		if envelope["outcome"] is not None and (not isinstance(envelope["outcome"], str) or KIND_RE.fullmatch(envelope["outcome"]) is None):
			raise MailboxError(f"message contains an invalid outcome: {path.name}")
		if envelope["responds_to"] is not None and (not isinstance(envelope["responds_to"], str) or CLAIMED_RE.fullmatch(envelope["responds_to"]) is None):
			raise MailboxError(f"message contains an invalid responds_to claim: {path.name}")
		if not isinstance(envelope["created_at"], str):
			raise MailboxError(f"message contains an invalid creation timestamp: {path.name}")
		_parse_timestamp(envelope["created_at"])
		if message_type == "notice":
			if not isinstance(envelope["expires_at"], str):
				raise MailboxError(f"notice contains an invalid expiration timestamp: {path.name}")
		...
		if retention == RETENTION_DURABLE:
			if not isinstance(envelope["target"], str):
				raise MailboxError(f"durable message target must be text: {path.name}")
			return envelope, self._target(envelope["target"])
		body = envelope["body"]
		if not isinstance(body, str) or not body.strip():
			raise MailboxError(f"transient message body must be non-empty text: {path.name}")
		try:
			body_bytes = body.encode("utf-8")
		except UnicodeError as exc:
			raise MailboxError(f"transient message body is not valid UTF-8: {path.name}") from exc
		if len(body_bytes) > TRANSIENT_BODY_MAX_BYTES:
			raise MailboxError(f"transient message body exceeds {TRANSIENT_BODY_MAX_BYTES} bytes: {path.name}")
		return envelope, None

	def _validate_message(self, path: Path) -> tuple[dict[str, Any], Path | None]:
		return self._validate_v5(path)

	@staticmethod
	def _content_sha256(envelope: dict[str, Any], target: Path | None) -> str:
		if envelope["retention"] == RETENTION_DURABLE:
			if target is None:
				raise MailboxError("durable message is missing its target")
			return _sha256_file(target)
		return _sha256_bytes(envelope["body"].encode("utf-8"))
	...
	def _write_claim_receipt(self, claim: Path, *, role: str, actor: str, seed: str | None, original: str, envelope: dict[str, Any], target: Path | None) -> Path:
		...
		data = {
			...
			"target_rel": envelope.get("target"),
			"claim_sha256": _sha256_file(claim),
			"content_sha256": self._content_sha256(envelope, target),
	...
	def _notice_seen(self, notice: Path, role: str, actor: str, seed: str | None) -> bool:
		...
		if data.get("notice") != notice.name or data.get("role") != role or data.get("actor") != actor or data.get("seed") != seed or data.get("retention") != envelope["retention"] or data.get("notice_sha256") != _sha256_file(notice) or data.get("content_sha256") != self._content_sha256(envelope, target):
	...
	def _unseen_notices(self, role: str, actor: str, seed: str | None) -> list[Path]:
		...
		for notice in self._notices():
			envelope, _ = self._validate_v5(notice)
	...
	def see(self, role: str, notice_name: str, *, actor: str | None, seed: str | None) -> dict[str, Any]:
		...
		if not self._notice_seen(notice, role, identity, live_seed):
			data = {"version": 2, "notice": notice_name, "role": role, "actor": identity, "seed": live_seed, "retention": envelope["retention"], "notice_sha256": _sha256_file(notice), "content_sha256": self._content_sha256(envelope, target), "seen_at": _utc_timestamp()}
			_atomic_publish_bytes(receipt, _json_bytes(data))
		return {"status": "notice", "notice": notice_name, "retention": envelope["retention"], "target": envelope.get("target"), "body": envelope.get("body"), "from": envelope["from_role"], "expires_at": envelope["expires_at"], "thread_id": envelope["thread_id"], "receipt": str(receipt)}
	...
	def _load_claim(self, role: str, claim_name: str, *, actor: str | None, seed: str | None) -> tuple[dict[str, Any], Path | None, Path, Path]:
		...
		if receipt.get("version") != 3 or receipt.get("claim") != claim_name or receipt.get("role") != role or receipt.get("repo_root") != str(self.repo_root) or receipt.get("actor") != identity or receipt.get("seed") != live_seed:
			raise MailboxError("claim receipt identity does not match this operation")
		if receipt.get("retention") != envelope["retention"] or receipt.get("target_rel") != envelope.get("target") or receipt.get("claim_sha256") != _sha256_file(claim) or receipt.get("content_sha256") != self._content_sha256(envelope, target):
			raise MailboxError("claim or authoritative content changed since claim; leaving claim intact")
	...
	@staticmethod
	def _transient_body(body: bytes) -> str:
		if not body.strip():
			raise MailboxError("message body is empty; refusing to publish")
		if len(body) > TRANSIENT_BODY_MAX_BYTES:
			raise MailboxError(f"transient message body exceeds {TRANSIENT_BODY_MAX_BYTES} bytes")
		try:
			return body.decode("utf-8")
		except UnicodeDecodeError as exc:
			raise MailboxError("transient message body must be valid UTF-8") from exc

	@staticmethod
	def _validate_message_metadata(*, retention: str, kind: str, thread_id: str, outcome: str | None, responds_to: str | None) -> None:
		if retention not in RETENTIONS:
			raise MailboxError(f"invalid retention policy: {retention!r}")
		if KIND_RE.fullmatch(kind) is None:
			raise MailboxError(f"invalid message kind: {kind!r}")
		if THREAD_RE.fullmatch(thread_id) is None:
			raise MailboxError(f"invalid thread id: {thread_id!r}")
		if outcome is not None and KIND_RE.fullmatch(outcome) is None:
			raise MailboxError(f"invalid message outcome: {outcome!r}")
		if responds_to is not None and CLAIMED_RE.fullmatch(responds_to) is None:
			raise MailboxError(f"invalid responds-to claim: {responds_to!r}")

	def _publish_envelope(self, *, role: str, to_role: str, actor: str, seed: str | None, retention: str, target: Path | None, body: str | None, timestamp: str, message_id: str, kind: str, thread_id: str, outcome: str | None, responds_to: str | None, ttl: float) -> Path:
		self._validate_message_metadata(retention=retention, kind=kind, thread_id=thread_id, outcome=outcome, responds_to=responds_to)
		if retention == RETENTION_DURABLE:
			if target is None or body is not None:
				raise MailboxError("durable messages require a target and cannot embed a body")
		else:
			if target is not None or body is None:
				raise MailboxError("transient messages require an embedded body and cannot name a target")
		...
		if retention == RETENTION_DURABLE:
			envelope["target"] = target.relative_to(self.work).as_posix()
		else:
			envelope["body"] = body
		if to_role == "ALL":
		...
		if to_role == "ALL":
			...
			_atomic_publish_bytes(receipt, _json_bytes({"version": 2, "notice": name, "role": role, "actor": actor, "seed": seed, "retention": retention, "notice_sha256": _sha256_file(token), "content_sha256": self._content_sha256(envelope, target), "published_at": _utc_timestamp()}))
		return token

	def send(self, role: str, to_role: str, destination_rel: str | None, body: bytes, *, actor: str | None, seed: str | None, retention: str, kind: str, thread_id: str | None, outcome: str | None, ttl: float) -> dict[str, Any]:
		identity, live_seed = self._identity(role, actor, seed)
		if to_role == "all":
			envelope_to = "ALL"
		...
		self._validate_message_metadata(retention=retention, kind=kind, thread_id=thread, outcome=outcome, responds_to=None)
		detail: Path | None = None
		body_text: str | None = None
		if retention == RETENTION_DURABLE:
			if destination_rel is None:
				raise MailboxError("durable send requires a destination beneath work/")
			destination = self._detail_dir(destination_rel)
			detail, timestamp, message_id = self._publish_detail(role, envelope_to, destination, body, actor=identity, kind=kind, thread_id=thread, outcome=outcome, responds_to=None)
		else:
			if destination_rel is not None:
				raise MailboxError("transient send does not accept a detail destination")
			body_text = self._transient_body(body)
			timestamp = _utc_timestamp()
			message_id = secrets.token_hex(6)
		token = self._publish_envelope(role=role, to_role=envelope_to, actor=identity, seed=live_seed, retention=retention, target=detail, body=body_text, timestamp=timestamp, message_id=message_id, kind=kind, thread_id=thread, outcome=outcome, responds_to=None, ttl=ttl)
		return {"status": "notice-published" if envelope_to == "ALL" else "handoff-published", "retention": retention, "detail": detail.relative_to(self.repo_root).as_posix() if detail is not None else None, "message": token.relative_to(self.repo_root).as_posix(), "from": role, "to": envelope_to, "thread_id": thread}

	def _response_destination(self, target: Path | None, destination_rel: str | None) -> Path:
		if destination_rel is None:
			if target is None:
				raise MailboxError("switching a transient message to durable retention requires --destination")
	...
	def respond(self, role: str, claim_name: str, body: bytes, *, actor: str | None, seed: str | None, close: bool, to_role: str | None, destination_rel: str | None, retention: str | None, kind: str, thread_id: str | None, outcome: str | None, ttl: float) -> dict[str, Any]:
		identity, live_seed = self._identity(role, actor, seed)
		...
		response_retention = retention or envelope["retention"]
		self._validate_message_metadata(retention=response_retention, kind=kind, thread_id=thread, outcome=outcome, responds_to=claim_name)
		detail: Path | None = None
		body_text: str | None = None
		if response_retention == RETENTION_DURABLE:
			...
			if destination_rel is not None:
				raise MailboxError("transient response does not accept --destination")
			body_text = self._transient_body(body)
			timestamp = _utc_timestamp()
			message_id = secrets.token_hex(6)
	...
	def expire(self, role: str, notice_name: str, *, actor: str | None, seed: str | None) -> dict[str, Any]:
		...
		if data.get("version") != 2 or data.get("notice") != notice_name or data.get("role") != role or data.get("actor") != identity or data.get("seed") != live_seed or data.get("retention") != envelope["retention"] or data.get("notice_sha256") != _sha256_file(notice) or data.get("content_sha256") != self._content_sha256(envelope, target):
...
def _parser() -> argparse.ArgumentParser:
	...
	parser.add_argument("--retention", choices=sorted(RETENTIONS), help="message retention (send default: durable; responses inherit the incoming message)")
...
def main(argv: list[str] | None = None) -> int:
	...
	try:
		...
		if ns.action == "scan":
			...
			retention = ns.retention or RETENTION_DURABLE
			if retention == RETENTION_DURABLE:
				if len(ns.args) not in (2, 3):
					raise MailboxError("durable send requires TO_ROLE DESTINATION [BODY_FILE|-]; stdin is the default")
				destination_rel = ns.args[1]
				body_arg = ns.args[2] if len(ns.args) == 3 else None
			else:
				if len(ns.args) not in (1, 2):
					raise MailboxError("transient send requires TO_ROLE [BODY_FILE|-]; stdin is the default")
				destination_rel = None
				body_arg = ns.args[1] if len(ns.args) == 2 else None
			body = _read_body(body_arg)
			result = box.send(ns.role, ns.args[0], destination_rel, body, actor=ns.actor, seed=ns.seed, retention=retention, kind=ns.kind, thread_id=ns.thread, outcome=ns.outcome, ttl=ns.ttl)
			...
			result = box.respond(ns.role, ns.args[0], body, actor=ns.actor, seed=ns.seed, close=ns.action == "close", to_role=ns.to, destination_rel=ns.destination, retention=ns.retention, kind=ns.kind, thread_id=ns.thread, outcome=ns.outcome, ttl=ns.ttl)
