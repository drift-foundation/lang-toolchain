...
"""
drift verify-app — verify a deployed APP artifact directory (verify only).

The app analog of `drift trust verify-package`: given an app deploy directory
(the runnable binary + signed author/cert/provenance sidecars, NO `.zdmp`),
verify the three-leg agreement (author == cert == provenance artifact_kind ==
"app"; cert artifact_path names the binary; sha256(binary) == cert ==
provenance; SCI agrees; signatures verify against the namespace-derived trust
subject).  Read-only — this command NEVER executes the binary.  App execution
is the orchestrator/service-manager's job after it consumes a verified result.
"""
...
def build_arg_parser() -> argparse.ArgumentParser:
	p = argparse.ArgumentParser(
		prog="drift verify-app",
		description=(
			"Verify a deployed APP directory end to end (binary + author/cert/"
			"provenance sidecars). Read-only: does NOT execute the binary."
		),
	)
	p.add_argument("app_dir", type=UserPath, help="Deployed app directory.")
	...
	g.add_argument("--allow-bundled-pubkey", action="store_true",
		help="Self-consistency only: verify against the app's OWN bundled pubkey.")
	p.add_argument("--expect-version", type=str, default=None,
		help="Assert the app version equals this value.")
...
def run(argv: list[str] | None = None) -> int:
	"""Entry point for `drift verify-app`.  0=verified, 1=failed, 2=usage."""
	...
	if args.json:
		...
		if report.get("package_id"):
			print(f"{verdict}: {report['package_id']}@{report['version']} ({report['trust_source']}) [app]")
		else:
			print(f"{verdict}: {report.get('package_dir')}")
		for m in report["modules"]:
			mark = "✓" if m["ok"] else "✗"
			detail = m["mode"] if m["ok"] else m["reason"]
			print(f"  {mark} subject {m['module_id']}: {detail}")
